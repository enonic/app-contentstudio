...
import com.enonic.app.contentstudio.json.schema.mixins.MixinDescriptorJson;
import com.enonic.app.contentstudio.json.schema.mixins.MixinsJson;
...
public final class MixinsContextResource
    implements JaxRsComponent
{
    ...
    public MixinsJson getContentXData( @QueryParam("contentId") final String id, @Context HttpServletRequest request )
    {
        ...
        if ( content.isSite() )
        {
            SiteConfigsDataSerializer.fromData( content.getData().getRoot() ).stream().map( SiteConfig::getApplicationKey ).forEach(
                applicationKeys::add );
        }
        else
        {
            siteConfigService.getSiteConfigs( content.getPath().getParentPath() ).stream().map( SiteConfig::getApplicationKey ).forEach(
                applicationKeys::add );
        }
    ...
    private List<MixinDescriptorJson> createMixinsJson( final MixinOptions mixinOptions, @Context HttpServletRequest request )
    {
        return mixinOptions.stream().map(
            mixinOption -> MixinDescriptorJson.create().setMixinDescriptor( mixinOption.mixinDescriptor() ).setLocaleMessageResolver(
