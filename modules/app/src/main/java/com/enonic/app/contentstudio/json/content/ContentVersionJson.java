...
public class ContentVersionJson
{
                               ...
                               final ContentPublishInfoResolver contentPublishInfoResolver )
    {
        ...
        this.publishInfo =
            version.actions().stream().filter( c -> c.operation().equals( ContentVersionHelper.PUBLISH_KEY ) ).findFirst().map( c -> {
                return new ContentPublishInfoJson(
        ...
        this.changes = version.actions()
            .stream()
            .map(
                c -> new ActionJson( c.operation(), c.fields(), c.editorial() != null ? c.editorial().toString() : null , c.user().toString(), resolveUserDisplayName( c.user(), principalsResolver ),
                                     c.opTime() ) )
            .toList();
    }
