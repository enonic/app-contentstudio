...
export const ItemLabel = ({
    ...
    return (
        <div
            ...
            className={cn('grid gap-2.5 items-center', icon ? 'grid-cols-[auto_1fr]' : 'grid-cols-1', className)}
            {...props}
        >
            {icon && (
                <div
                    className={cn(
                        'flex items-center justify-center shrink-0 group-data-[tone=inverse]:text-alt size-6',
                    )}
                >
                    {icon}
                </div>
            ...
            <div className="flex flex-col text-left overflow-hidden">
                <span className="font-semibold leading-5.5 truncate w-full group-data-[tone=inverse]:text-alt">
                    {primary}
                </span>
