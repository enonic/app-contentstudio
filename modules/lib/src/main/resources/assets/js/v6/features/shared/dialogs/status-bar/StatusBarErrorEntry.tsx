...
export const StatusBarErrorEntry = ({
    className,
    status,
    label,
    disabled,
    children,
}: StatusBarErrorEntryProps): React.ReactElement => {
    return (
        <StatusBarEntry
            data-component={STATUS_BAR_ERROR_ENTRY_NAME}
            className={cn(status === 'in-progress' ? 'bg-surface-warn' : 'bg-surface-error', className)}
        >
