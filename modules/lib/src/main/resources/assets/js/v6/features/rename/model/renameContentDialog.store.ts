...
function validateContentName(value: string): RenameContentDialogValidationStatus | undefined {
    ...
    if (RESERVED_CONTENT_NAMES.has(trimmedValue) || hasInvalidContentNameCharacters(trimmedValue)) {
        return 'invalid';
    }
...
export const openRenameContentDialog = ({
    ...
    const mode: RenameContentDialogMode = hasName ? (isPublished ? 'rename-published' : 'rename') : 'set-name';
...
export const setRenameContentDialogValue = (value: string): void => {
    ...
    $renameContentDialog.set({
        ...
        value,
        validationStatus,
        availabilityStatus: validationStatus === 'invalid' ? undefined : state.availabilityStatus,
    });
