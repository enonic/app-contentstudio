 ...
 */
export function isAncestorOf<T>(state: TreeState<T>, ancestorId: string, descendantId: string): boolean {
 ...
 */
export function isDescendantOf<T>(state: TreeState<T>, descendantId: string, ancestorId: string): boolean {
...
export function getRootNodes<T>(state: TreeState<T>): TreeNode<T>[] {
    return state.rootIds.map((id) => state.nodes.get(id)).filter((node): node is TreeNode<T> => node !== undefined);
}

/**
 * Finds nodes matching a predicate.
 */
export function findNodes<T>(state: TreeState<T>, predicate: (node: TreeNode<T>) => boolean): TreeNode<T>[] {
 ...
 */
export function findNode<T>(state: TreeState<T>, predicate: (node: TreeNode<T>) => boolean): TreeNode<T> | undefined {
    for (const node of state.nodes.values()) {
        if (predicate(node)) {
            return node;
        }
...
export function needsChildrenLoad<T>(state: TreeState<T>, id: string): boolean {
    ...
    return node.hasChildren && node.childIds.length === 0 && !state.loadingIds.has(id);
}
