...
import { getTriggerButtonId, getTriggerElement } from '../../../shared/lib/ckeditor/ckeditorDialogUtils';
...
const findHighlights = (editor: CKEDITOR.editor, state: SearchPopupStore): SearchHighlight[] => {
    const rootNode =
        (editor as CKEDITOR.editor & { _: { editable?: CKEDITOR.dom.element } })._?.editable ??
...
const refreshSearchPopup = (patch: Partial<Pick<SearchPopupStore, 'findValue' | 'matchCase' | 'wholeWords'>>): void => {
    const state = $searchPopup.get();
