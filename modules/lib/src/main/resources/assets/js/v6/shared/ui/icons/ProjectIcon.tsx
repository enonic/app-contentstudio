...
import { resolveProjectIconUrl } from '../../lib/url/projects';
...
export const ProjectIcon = ({ projectName, language, hasIcon, isLayer, className }: ProjectIconProps): ReactElement => {
    ...
    if (url) {
        return (
            <img
                ...
                alt={projectName}
                draggable={false}
                className={cn('size-8 rounded-full bg-center object-cover', className)}
            />
        );
    }
    ...
    if (!language) {
        if (isLayer) {
            return <Layers className={cn('size-8 flex items-center justify-center', className)} />;
        }
        return <DefaultProjectIcon className={cn('size-8 flex items-center justify-center', className)} />;
    }
