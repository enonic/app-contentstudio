...
export class PublishItemsListElement extends LegacyElement<typeof PublishItemsList, Props> {
    ...
    getItemsIds(): ContentId[] {
        return this.props
            .get()
            .items.filter((item) => !this.$excludedIds.get().includes(item.getContentId()))
    ...
    removeItems(items: ContentSummaryInput[], _?: boolean): void {
        ...
        this.props.setKey(
            'items',
            this.props.get().items.filter((item) => !removeIds.has(item.getId())),
        );
    }
    ...
    removeItemsByIds(ids: ContentId[]): void {
        this.props.setKey(
            'items',
            this.props.get().items.filter((item) => !ids.includes(item.getContentId())),
        );
    }
