 ...
 */
export function removeNode<T>(state: TreeState<T>, id: string, removeDescendants = true): TreeState<T> {
    ...
    if (removeDescendants) {
        ...
        if (grandParentId !== null) {
            ...
            if (grandParent) {
                const newChildIds = [...grandParent.childIds.filter((cid) => cid !== id), ...node.childIds];
    ...
    if (removeDescendants && grandParentId !== null) {
        ...
        if (parent) {
            const nextTotalChildren =
                parent.totalChildren == null ? parent.totalChildren : Math.max(0, parent.totalChildren - 1);
 ...
 */
export function removeNodes<T>(state: TreeState<T>, ids: string[], removeDescendants = true): TreeState<T> {
    ...
    for (const [parentId, removedChildIds] of parentsToUpdate) {
        ...
        if (parent) {
            ...
            const nextTotalChildren =
                parent.totalChildren == null ? parent.totalChildren : Math.max(0, parent.totalChildren - removedCount);
 ...
 */
export function setChildren<T>(state: TreeState<T>, parentId: string | null, childIds: string[]): TreeState<T> {
    if (parentId === null) {
 ...
 */
export function appendChildren<T>(state: TreeState<T>, parentId: string | null, childIds: string[]): TreeState<T> {
    if (parentId === null) {
 ...
 */
export function setLoading<T>(state: TreeState<T>, id: string | null, loading: boolean): TreeState<T> {
 ...
 */
export function setLoadingData<T>(state: TreeState<T>, ids: string[], loading: boolean): TreeState<T> {
    const loadingDataIds = new Set(state.loadingDataIds);
 ...
 */
export function moveNode<T>(state: TreeState<T>, id: string, newParentId: string | null, index?: number): TreeState<T> {
    const node = state.nodes.get(id);
    if (!node) return state;
