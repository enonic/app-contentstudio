...
type TrackedContentItemsChange<T extends TrackedContentItem, S extends TrackedContentItemsState<T>> = {
    state: S;
    changed: boolean;
    changedMain: boolean;
...
export function patchTrackedContentItems<T extends TrackedContentItem, S extends TrackedContentItemsState<T>>(
    state: S,
    updates: readonly T[],
...
export function removeTrackedContentItems<T extends TrackedContentItem, S extends TrackedContentItemsState<T>>(
    state: S,
    idsToRemove: Set<string>,
): TrackedContentItemsChange<T, S> {
    ...
    const exclusionsChanged =
        excludeChildrenIds.length !== state.excludeChildrenIds.length ||
