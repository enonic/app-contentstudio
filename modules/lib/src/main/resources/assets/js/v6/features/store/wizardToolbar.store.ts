...
export function resolveWizardToolbarContentFullPath(
    ...
    const currentPathName = contentPath || path.getName();
    const currentPath = ContentPath.create().fromParent(path.getParentPath(), currentPathName).build();
    return getContentPathDisplayValues(currentPath, unnamedContentPathLabel).fullPath;
}
