...
describe('toAiToolHelperPath', () => {
    ...
    it('encodes a mixin field', () => {
        expect(toAiToolHelperPath({ kind: 'mixin', mixin: 'com.enonic.app.foo:MyMixin', field: 'heading' })).toBe(
            '__mixins__/com.enonic.app.foo/MyMixin/heading',
        );
    });

    it('encodes a mixin with an empty field', () => {
        expect(toAiToolHelperPath({ kind: 'mixin', mixin: 'com.enonic.app.foo:MyMixin', field: '' })).toBe(
            '__mixins__/com.enonic.app.foo/MyMixin',
        );
    });
    ...
    it('encodes a component config field', () => {
        expect(toAiToolHelperPath({ kind: 'componentConfig', component: '/main/0', field: 'caption' })).toBe(
...
describe('toPluginContextPath', () => {
    ...
    it('returns null for kinds the plugin cannot resolve in its form model', () => {
        expect(
            toPluginContextPath({ kind: 'mixin', mixin: 'com.enonic.app.foo:MyMixin', field: 'heading' }),
        ).toBeNull();
