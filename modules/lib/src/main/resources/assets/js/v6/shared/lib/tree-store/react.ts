...
export function useTreeStore<T>(options: UseTreeStoreOptions<T> = {}): UseTreeStoreReturn<T> {
    ...
    const boundActions = useMemo(
        () => ({
            ...
            setNodes: (opts: CreateNodeOptions<T>[]) => setState(actions.setNodes(stateRef.current, opts)),
            setNodeData: (id: string, data: T) => setState(actions.setNodeData(stateRef.current, id, data)),
