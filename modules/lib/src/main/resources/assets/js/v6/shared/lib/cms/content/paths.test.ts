...
describe('content paths utils', () => {
    ...
    it('returns parent ids with created descendants', () => {
        const parentItems = [createPathAwareItem('1', '/site/parent'), createPathAwareItem('2', '/site/other')];
        const createdItems = [createPathItem('/site/parent/child'), createPathItem('/outside/item')];

        expect(findContentIdsWithCreatedDescendants(parentItems, createdItems).map((id) => id.toString())).toEqual([
            '1',
        ]);
    });
    ...
    it('normalizes unnamed elements in a content path', () => {
        expect(formatContentPath(createContentPath('/__unnamed__parent/juyjju'), unnamedPathLabel)).toBe(
            '/<unnamed>/juyjju',
        );
        expect(formatContentPath(createContentPath('/__unnamed__parent/__unnamed__child'), unnamedPathLabel)).toBe(
            '/<unnamed>/<unnamed>',
        );
    });
    ...
    it('normalizes the parent path but appends the leaf label verbatim', () => {
        ...
        expect(formatContentFullPath('/__unnamed__parent', unnamedPathLabel, unnamedPathLabel)).toBe(
            '/<unnamed>/<unnamed>',
        );
