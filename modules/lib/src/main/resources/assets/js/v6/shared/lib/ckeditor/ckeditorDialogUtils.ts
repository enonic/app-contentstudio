...
export const restoreEditorSelection = (editor: CKEDITOR.editor, bookmarks?: CkEditorBookmarks): void => {
    if (!bookmarks) {
        return;
    }
...
export const restoreEditorSelectionSafe = (editor: CKEDITOR.editor, bookmarks?: CkEditorBookmarks): void => {
    if (!bookmarks) {
        return;
    }
...
export const hideOriginalDialog = (dialog: CKEDITOR.dialog): void => {
    ...
    const backgroundCover = dialogElement?.ownerDocument.getElementsByClassName('cke_dialog_background_cover')[0] as
        | HTMLElement
        | undefined;
...
export const getDialogValue = (dialog: CKEDITOR.dialog, pageId: string, elementId: string): string => {
    return getDialogElement(dialog, pageId, elementId).getValue();
};

export const setDialogValue = (dialog: CKEDITOR.dialog, pageId: string, elementId: string, value: string): void => {
