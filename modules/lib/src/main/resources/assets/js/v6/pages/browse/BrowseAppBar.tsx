...
export const BrowseAppBar = (): ReactElement => {
    ...
    const title = showProjectTitle ? activeProjectName : appName || applicationName;

    return (
        <header className="bg-surface-neutral h-15.25 px-3.5 sm:pl-5 py-2 pr-24 flex items-center gap-2 border-b border-bdr-soft">
            <Toggle
            ...
            <div className="contents" inert={isMobileSidebarOpen} aria-hidden={isMobileSidebarOpen || undefined}>
                <h1
                    ...
                    className={cn(
                        ...
                        showProjectTitle ? 'block sm:hidden' : 'mr-auto sm:text-2xl',
                    )}
                >
                    {title}
                </h1>
                ...
                {!noProjectMode && hasProjectContext && (
                    <Button
                        className={cn('min-w-0 disabled:opacity-100 max-sm:hidden', showProjectTitle && 'mr-auto')}
                        size="sm"
