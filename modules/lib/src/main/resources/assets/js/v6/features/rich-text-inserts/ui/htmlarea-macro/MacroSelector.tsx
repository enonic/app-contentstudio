...
export const MacroSelector = (): ReactElement => {
    ...
    return (
        <div data-component={COMPONENT_NAME} className="flex flex-col gap-2">
            ...
            {hasSelection && <MacroSelectedView descriptor={selectedDescriptor} onRemove={deselectDescriptor} />}
                ...
                return (
                    <Listbox.Item key={key} value={key}>
                        <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            {macro.getIconUrl() && (
                                <img src={macro.getIconUrl()} alt="" className="size-6 shrink-0 object-contain" />
                            )}
                            <div className="min-w-0 flex-1">
                                <span className="block text-sm font-medium truncate">{macro.getDisplayName()}</span>
                                {macro.getDescription() && (
                                    <span className="block text-xs text-subtle truncate">{macro.getDescription()}</span>
                                )}
                            </div>
                        </div>
                    </Listbox.Item>
    ...
    return (
        <div className="flex items-center gap-2.5 min-w-0">
            {descriptor.getIconUrl() && (
                <img src={descriptor.getIconUrl()} alt="" className="size-8 shrink-0 object-contain" />
            )}
            <div className="min-w-0 flex-1">
