...
function summary({id = CONTENT_ID, modifiedMs, workflowState}: {
    id?: string;
    modifiedMs: number;
...
describe('wizardContentSync.service', () => {
    ...
    it('ignores events for other content', () => {
        emitContentUpdated([summary({id: 'other', modifiedMs: BASE_MODIFIED_MS, workflowState: WorkflowState.READY})]);
