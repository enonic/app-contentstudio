...
export const EditLockOverlay = ({
    locked,
    contentClassName,
    className,
    children,
    ...rest
}: EditLockOverlayProps): ReactElement => {
    return (
        <div className={cn('relative isolate', className)} {...rest} data-component={EDIT_LOCK_OVERLAY_NAME}>
            ...
            {locked && (
                <div className="pointer-events-none absolute inset-0 z-20 bg-surface-neutral/70" aria-hidden="true" />
            )}
        </div>
    );
};
