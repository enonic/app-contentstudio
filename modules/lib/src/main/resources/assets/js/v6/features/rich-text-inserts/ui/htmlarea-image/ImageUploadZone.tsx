...
import { uploadMediaFile } from '../../../../entities/content/api/uploadMedia.api';
...
export const ImageUploadZone = (): ReactElement => {
    ...
    return (
        <>
            <DropZone icon={<Image size={28} />} hint={hintLabel} accept="image/*" onFiles={handleFiles} />
            {uploadError && <div className="text-sm text-error">{uploadError}</div>}
        </>
    );
};
