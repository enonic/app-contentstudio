...
export class AggregationsDisplayNamesResolver {
    ...
    updateAggregationsDisplayNames(aggregations: BucketAggregation[]): Q.Promise<void> {
        ...
        aggregations
            .filter((aggr) => this.isPrincipalAggregation(aggr.getName()))
            .forEach((principalAggr: BucketAggregation) => {
                updatePromises.push(
                    this.updateUnknownPrincipals(principalAggr).then(() => this.updateKnownPrincipals(principalAggr)),
                );
            });
    ...
    private updateWorkflowAggregations(aggregations: Aggregation[]) {
        const workflowAggr: Aggregation = aggregations.find(
            (aggr: Aggregation) => aggr.getName() === ContentAggregation.WORKFLOW.toString(),
        );
    ...
    updateKnownPrincipals(principalsAggregation: BucketAggregation): void {
        principalsAggregation.getBuckets().forEach((bucket: Bucket) => {
            const displayName: string =
                bucket.getKey() === this.currentUserId
    ...
    updateUnknownPrincipals(principalsAggregation: BucketAggregation): Q.Promise<void> {
        ...
        const unknownPrincipals: PrincipalKey[] = principalsAggregation
            .getBuckets()
        ...
        return this.getPrincipalsByKeys(unknownPrincipals).then((principals: Principal[]) => {
            principals.forEach((principal: Principal) => {
                this.principals.set(
                    principal.getKey().toString(),
                    principal.getDisplayName() || principal.getKey().toString(),
                );
            });
        });
    }
    ...
    updateLanguageAggregations(aggregations: Aggregation[]): Q.Promise<void> {
        const langAggr: Aggregation = aggregations.find(
            (aggr: Aggregation) => aggr.getName() === ContentAggregation.LANGUAGE.toString(),
        );
    ...
    private updateLanguageAggregation(langAggr: BucketAggregation): Q.Promise<void> {
        if (this.locales) {
            langAggr.getBuckets().forEach((bucket: Bucket) => {
                const displayName: string = this.locales
                    .find((locale: Locale) => locale.getId().toLowerCase() === bucket.getKey())
                    ?.getDisplayName();
    ...
    updateContentTypeAggregations(aggregations: Aggregation[]): Q.Promise<void> {
        const contentTypeAggr: Aggregation = aggregations.find(
            (aggr: Aggregation) => aggr.getName() === ContentAggregation.CONTENT_TYPE.toString(),
        );

        return contentTypeAggr
            ? this.updateContentTypeAggregation(contentTypeAggr as BucketAggregation)
            : Q.resolve(null);
    }
