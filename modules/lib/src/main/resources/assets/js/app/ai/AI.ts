...
export class AI {
    ...
    private getAiHelperByPath(path: string): AiHelper | undefined {
        ...
        return AiHelper.getAiHelpersByContext('data').find((helper: AiHelper) => helper.getDataPath() === path);
    }
    ...
    private getAiHelperByXData(path: string): AiHelper | undefined {
        ...
        return xData?.xDataStepForm ? AiHelper.getAiHelpersByContext('xdata').find(
    ...
    private getAiHelperByPage(path: string): AiHelper | undefined {
        if (path.indexOf(AI.CONFIG_PREFIX) > -1) {
            ...
            return AiHelper.getAiHelpersByContext('page').find((helper: AiHelper) => helper.getDataPath() === dataPath);
        }
