...
export class ProjectDAGVisualization extends DivEl{
    ...
    private static IMG_LEFT = 10;
    private static IMG_SIZE = 25;
    ...
    private setData(projectId: string = ''): Q.Promise<void> {
        const getIconHTML = (language: string) => {
            ...
            if (className !== 'fi-unknown') {
                ...
                style = `
                    width: ${ProjectDAGVisualization.IMG_SIZE}px;
                    height: ${ProjectDAGVisualization.IMG_SIZE}px;
                `;                
            } else {
                className = 'icon-tree-2';
                style = `
                    color: #53585F;
                    font-size: ${ProjectDAGVisualization.IMG_SIZE}px;
                `;
            }
        ...
        return this.loadAllProjects().then(() => {
            ...
            this.data = this.allProjects.map(project => ({
                ...
                iconHTML: getIconHTML(project.getLanguage()),
    ...
    private plotFlags(svg: D3SVGG): D3SVGG {
        const left = ProjectDAGVisualization.IMG_LEFT;
        const top = (ProjectDAGVisualization.RECT_HEIGHT - ProjectDAGVisualization.IMG_SIZE) / 2;
