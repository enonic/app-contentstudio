...
import {SagaPostRequest, SagaPostRequestResult} from '../../../../saga/SagaPostRequest';
import * as Q from 'q';
...
export class SagaWidgetItemView
    ...
    private askAssistant(command?: string): void {
        ...
        SagaCommands.expandText(SagaCommandProcessor.convertToAssistantMessage(commandText, this.data.html), this.data.html).then((result: SagaGetRequestResult) => {
            console.log('html', result.data);

            console.log('response', result);

         //   return this.waitForComplete(result.id).then((result: string) => {
                interaction.addAssistantMessage(result.data);
     //       });
    ...
    private waitForComplete(id: string): Q.Promise<string> {
        return new SagaPostRequest({id}).sendAndParse().then((result: SagaPostRequestResult) => {
            if (result.status === 'completed') {
                ...
                setTimeout(() => {
                    defer.resolve(this.waitForComplete(id));
                }, 1000);
