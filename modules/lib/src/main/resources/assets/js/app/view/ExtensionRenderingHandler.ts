...
export class ExtensionRenderingHandler {
    ...
    private async doRender(summary: ContentSummary, selectedMode: Extension): Promise<RenderResult> {
        ...
        const isAuto = selectedMode.getDescriptorKey().getName() === PreviewModeDropdown.PREVIEW_AUTO;
        const items = isAuto ? this.renderer.getExtensionSelector().getAutoModeExtensions() : [selectedMode];
