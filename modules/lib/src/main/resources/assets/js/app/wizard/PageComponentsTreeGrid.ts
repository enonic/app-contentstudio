...
export class PageComponentsTreeGrid
    ...
    protected initListeners(): void {
        const invalidateHandler = this.setInvalid.bind(this);

        this.onDataChanged(invalidateHandler);
        this.onLoaded(invalidateHandler);
    }
    ...
    private setInvalid() {
        const invalidNodes: TreeNode<ComponentsTreeItem>[] = this.getRoot().getAllDefaultRootNodes()
            .filter((node) => node.getData()?.isInvalid());
        let stylesHash = {};

        invalidNodes.forEach((node: TreeNode<ComponentsTreeItem>) => {
