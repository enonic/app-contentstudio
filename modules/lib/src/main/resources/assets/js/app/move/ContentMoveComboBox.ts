...
export class ContentMoveComboBox extends ContentSelectorDropdown {
    ...
    protected initListeners(): void {
        ...
        this.listBox.onItemsAdded((items: ContentTreeSelectorItem[]) => {
            items.forEach((item: ContentTreeSelectorItem) => {
                this.listBox
                    .getItemView(item)
                    .toggleClass('readonly', this.readonlyChecker.isReadOnly(item.getContentSummary()));
            });
        });
    }
    ...
    setFilterContents(contents: ContentSummary[]): void {
        ...
        const isSameParent =
            contentsPaths.length < 2 ||
            contentsPaths.every((path, index, paths) => {
    ...
    protected loadListOnShown(): void {
        if (!this.loader.isLoading()) {
            // when listbox is shown first time it also triggers loading
            this.search();
        }
    }
...
class MoveReadOnlyChecker {
    ...
    private filterContentTypes: ContentTypeName[] = [
