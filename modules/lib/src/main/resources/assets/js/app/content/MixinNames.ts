...
import {type MixinDescriptor} from './MixinDescriptor';
...
export class MixinNames
    ...
    contains(mixinName: MixinName): boolean {
        let containName = this.array.some((curMixinName: MixinName) => {
            return curMixinName.equals(mixinName);
        });
        return !!containName;
    }
...
export class MixinNamesBuilder {
    ...
    fromMixinDescriptors(descriptors: MixinDescriptor[]): MixinNamesBuilder {
        if (!!descriptors) {
            descriptors.forEach((descriptor: MixinDescriptor) => {
                this.addMixinName(descriptor.getMixinName());
            });
        }
        return this;
    }
