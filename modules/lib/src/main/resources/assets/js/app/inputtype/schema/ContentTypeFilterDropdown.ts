...
export class ContentTypeFilterDropdown extends FilterableListBoxWrapperWithSelectedView<ContentTypeSummary> {
    ...
    protected loadListOnShown(): void {
        ...
        this.loader
            ...
            .then((contentTypes: ContentTypeSummary[]) => {
                ...
                if (this.listBox.isVisible() && this.optionFilterInput.getValue()) {
                    // filtering loaded items with search string if present
    ...
    createSelectedOption(item: ContentTypeSummary): Option<ContentTypeSummary> {
        return Option.create<ContentTypeSummary>().setValue(item.getId()).setDisplayValue(item).build();
    }
