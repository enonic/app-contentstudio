...
export class FragmentComponentView
    ...
    private convertTextComponentImageUrls(element: Element) {
        const content: ContentSummary = ContentContext.get().getContent()?.getContentSummary();

        if (content instanceof Content) {
            const thisFragmentComponent: PageItem = content.getPage().getComponentByPath(this.getPath());

            if (thisFragmentComponent instanceof FragmentComponent && thisFragmentComponent.hasFragment()) {
                const text = HTMLAreaHelper.convertRenderSrcToPreviewSrc(element.getHtml(), thisFragmentComponent.getFragment().toString());
