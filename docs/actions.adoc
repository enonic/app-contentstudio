...
TIP: It's possible to overrule the default behaviour of including all the dependencies by using the Content Studio's <<config#exclude_optional_dependencies, config file>>.
In this case only mandatory dependencies will be included, while optional can be added from the "Show excluded" mode.
