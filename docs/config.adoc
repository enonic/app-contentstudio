...
The *inline view* (shown as you select content in the tree) additionally injects the page editor, so Content Studio enforces `script-src 'self'` for it and `frame-ancestors 'self'` so the panel can frame it. Those two are enforced directly, independent of the config below — configuration must not be able to break the editor. The *preview* (a separate, explicit tab) injects nothing and only gap-fills.
