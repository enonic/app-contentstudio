...
When content is rendered inside Content Studio — the inline view shown as you select content in the tree, and the standalone preview — a configurable baseline Content Security Policy is applied. The baseline is filled in per directive: a page that ships its own CSP keeps every directive it declares (so an application that legitimately loads an external script keeps working), and only the baseline directives the page left out are added to contain unprotected content. A page with no CSP of its own gets the full baseline.
