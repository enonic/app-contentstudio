...
When content is previewed, the page's own Content Security Policy applies together with a configurable baseline. The baseline limits what previewed content may do, but never allows more than the page's own policy does — a page with a policy stricter than the baseline renders exactly as on the live site.
