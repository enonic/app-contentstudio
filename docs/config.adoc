...
[source,properties]
----
site.preview.contentSecurityPolicy=default-src 'self'; base-uri 'self'; form-action 'self'; script-src 'self'; object-src 'none'; img-src * data:; style-src * 'unsafe-inline'; font-src * data:
----

The value above is the default. Set the property to an empty value to disable the baseline and trust each page's own policy alone. Do not put `'unsafe-inline'` into the baseline's `script-src`: the request nonce added to that directive makes browsers ignore `'unsafe-inline'`.
