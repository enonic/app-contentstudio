...
When content is rendered inside Content Studio, a configurable baseline Content Security Policy is applied — but the two contexts treat it differently:

* The *inline view* (shown as you select content in the tree) injects the page editor, so it **forces** the baseline in: every baseline directive is unioned onto the page's policy, guaranteeing the editor's own `script-src 'self'` holds even over a page that declared, say, `script-src 'none'`. A good app's own sources (an external `script-src` host, …) survive the union.
* The *preview* (a separate, explicit tab) injects nothing, so it **gap-fills**: a page that ships its own CSP keeps every directive it declared untouched (a good app that legitimately loads an external script keeps working, and a page's strict `script-src 'none'` is never loosened); only the directives the page omitted are added, to contain unprotected content. A page with no CSP of its own gets the full baseline.
