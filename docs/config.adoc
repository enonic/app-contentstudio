...
The value above is the default: images, fonts, styles, media and framed embeds (YouTube, Vimeo, …) are allowed from anywhere, while scripts must come from the site itself or carry the request nonce. A player script that fetches streams from a CDN additionally needs the CDN host in `connect-src`.
