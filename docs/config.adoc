...
When content is rendered inside Content Studio — both the inline view shown as you select content in the tree and the standalone preview — the page's own Content Security Policy applies together with a configurable baseline. The baseline limits what that content may do, but never allows more than the page's own policy does — a page with a policy stricter than the baseline renders exactly as on the live site.
...
The value above is the default: it contains the directives that let previewed content run code or send data — scripts and connections are limited to the site itself (or the request nonce), forms and `+<base>+` to the site, and plugins are blocked — while everything visual (images, fonts, styles, media, framed embeds such as YouTube or Vimeo) is left to the page's own policy and renders as on the live site. To let a player script reach a CDN, add the CDN host to `script-src`/`connect-src` here.
