...
It's now possible to turn on a config for a content type to specify that no new content can be created for it. This will
effectively remove the content type from the list in the "New Content" dialog.

=== Relationship between content types

It's now possible to specify what content types are allowed as children of a certain content type. This setting will limit the
list of content types shown in the "New Content" dialog by showing only the allowed child content types for a selected content item.

=== Relationship between page components and content types
...
A new property in the Content Studio's config file can be used to hide the "Mark as Ready" action in the "Publishing Wizard" dialog to
disallow bulk update of selected items marking them as ready for publishing.
