...
To create a new layer, click *New* button in the Settings section and select "_Layer_" on the
"Project type" step of the New Project wizard. You will be required to select a parent project
(or another layer) which data in the new layer will be synchronised with.

image::new-project-dialog-1.png[Create Layer, 832]

IMPORTANT: You will not be able to select or change the parent project/layer once the new layer is created.

TIP: On the steps of the New Project wizard you can copy settings like Language, Access Mode and Roles from the parent project/layer by using the *Copy from parent* button.
If the parent doesn't have those properties set, or they are the same as in this layer, the button will be disabled.

image::new-project-dialog-2.png[Create Layer, 832]
...
image::project-context.png[Context switch, 420]
