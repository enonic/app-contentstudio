...
The page with the automatic template is disabled for editing. Clicking on it, or inside the <<#_components-view,Components View>> will show the menu with the option to enable the editing.
...
Selecting a component in the tree will highlight the component in the page editor and display its configuration in the <<../widgets#insert-tab,Components>> widget. The triangle on the right of each component will open a menu of options that can be selected for the component. This menu completely replicates the menu, described in the <<#_editing-page,previous>> section.
