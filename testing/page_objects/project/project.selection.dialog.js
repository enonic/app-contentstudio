...
class ProjectSelectionDialog extends Page {
    ...
    async clickOnCloseButton() {
        try {
            ...
            await this.handleError(
                'Project Selection dialog, Error occurred while clicking the Close button ',
                'err_proj_sel_close_button',
                err,
            );
        }
    }
    ...
    async waitForDialogLoaded() {
        try {
            ...
            await this.handleError(
                'Project Selection dialog, Error occurred while waiting for the dialog to be loaded',
                'err_wait_proj_sel_dialog',
                err,
            );
        }
    }
    ...
    async waitForDialogClosed() {
        try {
            ...
            await this.handleError(
                'Project Selection dialog, Error occurred while waiting for the dialog to be closed',
                err,
            );
        }
    }
    ...
    async selectContext(projectDisplayName) {
        try {
            ...
            await this.handleError(
                'Project Selection Dialog, tried to select the item',
                'err_select_project_context',
                err,
            );
        }
    }
    ...
    async getProjectsDisplayName() {
        let locator = XPATH.container + XPATH.projectList + TREE_GRID.PROJECT_LABEL_DISPLAY_NAME_SPAN;
    ...
    async getProjectLanguage(projectDisplayName) {
        let locator =
            XPATH.container +
            XPATH.projectList +
            XPATH.projectListItemByDisplayName(projectDisplayName) +
            '//span/span';
