...
class CompareContentVersionsDialog extends Page {
    ...
    async clickOnLeftRevertButton() {
        await this.waitForLeftRevertButtonDisplayed();
        return await this.clickOnElement(this.leftRevertButton);
    }

    async waitForLeftRevertButtonDisplayed() {
        return await this.waitForElementDisplayed(this.leftRevertButton, appConst.TIMEOUT_2);
    }

    async waitForRightRevertButtonDisplayed() {
        return await this.waitForElementDisplayed(this.leftRevertButton, appConst.TIMEOUT_2);
    }

    async waitForRightRevertButtonDisabled() {
        return await this.waitForElementDisabled(this.rightRevertButton, appConst.TIMEOUT_2);
    }
    async waitForLeftRevertButtonEnabled() {
        return await this.waitForElementEnabled(this.leftRevertButton, appConst.TIMEOUT_2);
    }


    async clickOnRightRevertButton() {
        await this.waitForElementDisplayed(this.leftRevertButton, appConst.TIMEOUT_2);
        return await this.clickOnElement(this.leftRevertButton);
    }
