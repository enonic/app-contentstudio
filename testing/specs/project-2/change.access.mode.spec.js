...
describe('change.access.mode.spec - Update Access Mode in project wizard', function () {
    ...
    it(`Preconditions: new project with 'Private' access mode should be added`, async () => {
    ...
    it('Precondition: new folder should be added in existing project(Private mode access)', async () => {
    ...
    it("GIVEN existing project(Private access mode) is opened WHEN access mode has been switched to 'Public' THEN Access Mode gets 'Public'", async () => {
        ...
        let actualMessages = await settingsBrowsePanel.waitForNotificationMessages();
        ...
        assert.ok(
            actualMessages.includes(appConst.projectModifiedMessage(PROJECT_DISPLAY_NAME)),
            'Expected message should appears',
        );
    });

    it(`GIVEN existing project's context is selected WHEN existing folder has been clicked THEN 'Everyone can read this item' header should be displayed in DetailsWidgetPermissionsSection`, async () => {
        ...
        assert.equal(
            actualDescription,
            appConst.ACCESS_WIDGET_HEADER.EVERYONE_CAN_READ,
