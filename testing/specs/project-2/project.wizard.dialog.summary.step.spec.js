...
describe('project.wizard.dialog.summary.step.spec - ui-tests for Summary wizard step', function () {
    ...
    it(`WHEN Summary step wizard is loaded THEN expected parameters should be displayed`, async () => {
        ...
        assert.ok(
            actualProjectName.includes(PROJECT_DISPLAY_NAME),
            'Expected project name should be displayed in the Step',
        );
        ...
        // Verify bug -  Description field is not displayed on Summary step #10628
        let actualDescription = await summaryStep.getDescription();
        assert.equal(actualDescription, 'test', 'Expected description should be displayed');
