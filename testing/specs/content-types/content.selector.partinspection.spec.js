/**
 * Created on 29.09.2023  updated on 25.06.2026
 */
...
describe('my.first.site.country.spec - Create a site with country content', function () {
    ...
    it(`Precondition: new site with 'main region' controller should be added`,
        async () => {
            ...
            SITE = contentBuilder.buildSite(displayName, null, [appConst.MY_FIRST_APP, appConst.TEST_APPS_NAME.APP_CONTENT_TYPES],
                'main region');
    ...
    it(`GIVEN existing site is opened WHEN Inspect panel with image selector has been opened THEN expected options should be displayed in flat mode in the expanded combobox`,
        async () => {
            ...
            await cityCreationPartInspectionPanel.typeNameAndSelectPart(appConst.PART_NAME.MY_FIRST_APP_CITY_CREATION);
            // 5. Click on mode-toggle and switch the image-selector to tree mode:
            await cityCreationPartInspectionPanel.clickOnImageSelectorModeTogglerButton();
            // 6. Verify that expected options are displayed in tree mode:
