...
describe('htmlarea1_0.cke.spec:  html area with CKE`', function () {
    ...
    await
    contentWizard.typeDisplayName('test_area0_1');
    await
    ...
    studioUtils.selectSiteAndOpenNewWizard(SITE.displayName, 'htmlarea0_1');
    await
    htmlAreaForm.typeTextInHtmlArea(TEXT_TO_TYPE)
    let result = await
    ...
    await
    contentWizard.typeData(htmlAreaContent);
    await
    contentWizard.waitAndClickOnSave();
    let expectedMessage = '\"' + htmlAreaContent.displayName + '\"' + ' is saved';
    await
    contentWizard.waitForExpectedNotificationMessage(expectedMessage);
})
    ;
    ...
    it(`GIVEN existing 'htmlArea 0:1' WHEN it has been reopened THEN expected text should be displayed in the area`,
        async() = > {
            let htmlAreaForm = new HtmlAreaForm();
    await
    studioUtils.selectContentAndOpenWizard(htmlAreaContent.displayName);
    let result = await
    htmlAreaForm.getTextFromHtmlArea();
    ...
    studioUtils.selectContentAndOpenWizard(htmlAreaContent.displayName);
    await
    htmlAreaForm.clickOnFullScreenButton();
    await
    fullScreenDialog.waitForDialogLoaded();
    let result = await
    fullScreenDialog.getTextFromHtmlArea();
    ...
    studioUtils.selectContentAndOpenWizard(htmlAreaContent.displayName);
    await
    htmlAreaForm.clickOnSourceButton();
    await
    sourceCodeDialog.waitForDialogLoaded();
    let result = await
    sourceCodeDialog.getText();
    ...
    sourceCodeDialog.waitForDialogLoaded();
    await
    sourceCodeDialog.typeText("");
    await
    sourceCodeDialog.clickOnOkButton();
    let result = await
    htmlAreaForm.getTextFromHtmlArea();
    ...
    // click on ESC:
    await
    fullScreenDialog.pressEscKey();
    //'full screen dialog should be closed:
    await
    fullScreenDialog.waitForDialogClosed();
        });
