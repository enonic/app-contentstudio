...
const DateForm = require('../../page_objects/wizardpanel/date.form.panel');
...
describe('datetime.config.spec: tests for datetime content ', function () {
    ...
    it('GIVEN wizard for new Date(1:1) is opened WHEN incorrect date has been typed THEN date content should be invalid', async () => {
        let dateForm = new DateForm();
        let contentWizard = new ContentWizard();
        // 1. Open wizard for new date 1:1
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.DATE_1_1);
        ...
        await dateForm.waitForOccurrenceValidationRecordingDisplayedAt(
            0,
            appConst.VALIDATION_MESSAGE.INVALID_VALUE_ENTERED,
        );
    });
    ...
    it('GIVEN wizard for new DateTime(1:1) with timezone is opened WHEN date time input has been clicked THEN date time picker popup dialog with timezone gets visible', async () => {
        ...
        let timezone = await dateTimePickerPopup.getTimezone();
        assert.ok(
            /^UTC[+-]\d{2}:\d{2}$/.test(timezone),
            `Timezone should be displayed as 'UTC(+|-)hh:mm' but was '${timezone}'`,
        );
        // 4. Verify that the time selectors are set to the current time (the input is empty):
        let time = await dateTimePickerPopup.getTime();
        assert.ok(/^\d{2}:\d{2}$/.test(time), `Time should be displayed as 'hh:mm' but was '${time}'`);
        let now = new Date();
        let [hours, minutes] = time.split(':').map(Number);
        let diffMinutes = Math.abs(hours * 60 + minutes - (now.getHours() * 60 + now.getMinutes()));
        assert.ok(
            Math.min(diffMinutes, 24 * 60 - diffMinutes) <= 5,
            `Time in the popup ('${time}') should be close to the current time`,
        );
        // 5. Verify that the calendar shows the current month and year:
        let month = await dateTimePickerPopup.getMonth();
        let expectedMonth = now.toLocaleString('en-US', { month: 'short' });
        assert.ok(
            month.toLowerCase().startsWith(expectedMonth.toLowerCase()),
            `Current month ('${expectedMonth}') should be displayed in the month selector but was '${month}'`,
        );
        let year = await dateTimePickerPopup.getYear();
        assert.equal(year, String(now.getFullYear()), 'Current year should be displayed in the year selector');
        // 6. Verify that 'OK' button is disabled while no date is selected:
        await dateTimePickerPopup.waitForOkButtonDisabled();
        // 7. Click on the current day in the calendar and verify that 'OK' button gets enabled:
        await dateTimePickerPopup.clickOnDayInCalendar(now.getDate());
    ...
    it('GIVEN wizard for new DateTime(1:1) opened WHEN only name input is filled in AND Save button has been pressed THEN the content should not be valid', async () => {
        ...
        assert.equal(
            actualMessage,
            appConst.VALIDATION_MESSAGE.THIS_FIELD_IS_REQUIRED,
    ...
    it("GIVEN wizard for new DateTime(1:1) opened WHEN name and valid date time have been typed in AND 'Save' button has been pressed THEN the content should be valid", async () => {
        ...
        assert.ok(
            isInValid === false,
            'The content should be valid, because correct value inserted in the required input',
        );
        // 5. Verify that default action is 'Mark as Ready' now
        await contentWizard.waitForMarkAsReadyButtonVisible();
    });
    ...
    it('GIVEN wizard for new DateTime(1:1) with tz is opened WHEN not valid date time has been typed THEN red border appears in the input AND the content should be not valid', async () => {
        ...
        await dateTimeForm.waitForOccurrenceValidationRecordingDisplayedAt(
            0,
            appConst.VALIDATION_MESSAGE.INVALID_VALUE_ENTERED,
        );
        // 4. Verify that the content is invalid:
    ...
    it("GIVEN wizard for not required 'Time 0:1' is opened WHEN time in incorrect format has been typed THEN 'Publish' menu item should be enabled, because the input is not required", async () => {
        ...
        await timeForm.waitForOccurrenceValidationRecordingDisplayedAt(
            0,
            appConst.VALIDATION_MESSAGE.INVALID_VALUE_ENTERED,
        );
        // 4. Verify that the content is invalid:
