...
describe('optionset.confirmation.spec: check for `confirmation` when deleting existing or new item-set `', function () {
    ...
    await
    studioUtils.selectSiteAndOpenNewWizard(SITE.displayName, 'optionset');
    //2. Click on the radio button and add new item-set:
    await
    optionSetForm.clickOnOption1Radio();
    await
    singleSelectionOptionSet.clickOnAddItemSetButton();
    studioUtils.saveScreenshot('new_item_set_added');
    await
    singleSelectionOptionSet.typeOptionName("test option");
    //3. Type a text in the second item-set:
    await
    singleSelectionOptionSet.typeInLabelInput("label1", 1);
    //3. Click on 'remove-icon' and try to close the second item-set:
    await
    singleSelectionOptionSet.clickOnRemoveItemSetOccurrenceView(1);
    ...
    await
    contentWizard.waitForSavedButtonVisible();
        });
