...
describe('publish.request.create.close.spec - request publish dialog - open and clothe this request', function () {
    ...
    it(`GIVEN new folder(Ready to Publish) is added and selected WHEN new Publish Request has been created THEN 'Request Details Dialog' should appear`, async () => {
        ...
        assert.equal(
            message,
            appConst.NOTIFICATION_MESSAGES.REQUEST_CREATED_MESSAGE,
        ...
        assert.ok(
            result.includes('/' + TEST_FOLDER1.displayName),
            'Expected item to publish should be present in the dialog',
        );
    });
    ...
    it(`GIVEN existing Publish Request WHEN Request Details dialog is opened AND 'Publish Now' button has been pressed THEN modal dialog closes and this request closes`, async () => {
        let publishRequestDetailsDialog = new PublishRequestDetailsDialog();
        let issueListDialog = new IssueListDialog();
        let browsePanel = new ContentBrowsePanel();
    ...
    it(`GIVEN issues list dialog is opened AND navigated to closed issues-tab WHEN existing closed request has been reopened THEN the request gets 'Open' AND 'no items to publish' should appear`, async () => {
        let browsePanel = new ContentBrowsePanel();
        let issueListDialog = new IssueListDialog();
        let publishRequestDetailsDialog = new PublishRequestDetailsDialog();
        await studioUtils.openIssuesListDialog();
    ...
    it(`GIVEN existing 'Open' request WHEN text in comments-area has been typed THEN 'Comment & Close Request' button gets visible`, async () => {
        let issueListDialog = new IssueListDialog();
