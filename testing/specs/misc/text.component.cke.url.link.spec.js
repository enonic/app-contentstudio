...
describe('Text Component with CKE - insert URL link specification', function () {
    ...
    it(`GIVEN 'Insert Link' dialog is opened WHEN invalid 'url' has been typed AND 'Insert' button pressed THEN validation message should appear`, async () => {
        ...
        assert.equal(
            ...
            'Invalid value entered - should be visible',
        );
    });

    it(`GIVEN Text component is inserted AND 'Insert Link' dialog is opened WHEN 'url-link' has been inserted THEN expected URL should appear in LiveEdit`, async () => {
        let contentWizard = new ContentWizard();
        let pageComponentView = new PageComponentView();
        let textComponentInspectionPanel = new TextComponentInspectionPanel();
        // 1. Open the site:
        ...
        await textComponentInspectionPanel.switchToParentFrame();
        await contentWizard.waitAndClickOnSave();
        await contentWizard.waitForNotificationMessage();
    });

    it(`GIVEN site is selected WHEN 'Automatic' is selected AND 'Preview' button has been pressed AND inserted link has been clicked THEN 'Enonic' site should be loaded in the page`, async () => {
    ...
    it('WHEN site is selected THEN the link should appear in Preview Panel', async () => {
        let contentItemPreviewPanel = new ContentItemPreviewPanel();
        await studioUtils.findAndSelectItem(SITE.displayName);
    ...
    it('GIVEN site is selected WHEN the link has been clicked THEN error message should appear in Preview Panel', async () => {
        let contentItemPreviewPanel = new ContentItemPreviewPanel();
        await studioUtils.findAndSelectItem(SITE.displayName);
