...
describe("insert.image.dlg.custom.width.spec: click on the `custom width` checkbox and check `image range value`", function () {
    ...
    it(`GIVEN htmlarea-content, 'Insert Image' dialog is opened AND an image is selected WHEN 'Custom width' checkbox should be unchecked by default`, async () => {
        ...
        await studioUtils.saveScreenshot(
            "image_dialog_custom_width_default_value",
        );
        assert.ok(
            isChecked === false,
            "'Custom width' checkbox should be unchecked by default",
        );
    ...
    it(`GIVEN htmlarea-content, 'Insert Image' dialog is opened AND an image is selected WHEN 'Custom width' checkbox has been clicked THEN default range(100%) for custom width should appear`, async () => {
        let insertImageDialog = new InsertImageDialog();
        let htmlAreaForm = new HtmlAreaForm();
        // 1. Open new wizard and open Insert Image dialog:
        await studioUtils.selectSiteAndOpenNewWizard(
    ...
    it(`GIVEN image withs custom width is inserted WHEN Save button has been pressed THEN content is saving`, async () => {
        let insertImageDialog = new InsertImageDialog();
        let htmlAreaForm = new HtmlAreaForm();
    ...
    it(`GIVEN existing htmlarea-content with image(custom width) is opened WHEN double click in htmmlarea THEN expected range should be displayed`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
        let insertImageDialog = new InsertImageDialog();
        ...
        await studioUtils.saveScreenshot(
            "image_dialog_custom_width_clicked_saved",
        );
        assert.equal(
            rangeValue,
            "100%",
            "Expected range should be in the dialog",
        );
    ...
    it(`GIVEN existing htmlarea-content with inserted image(custom width) is opened WHEN 'Custom Width' has been unselected THEN image-range gets hidden`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
        let insertImageDialog = new InsertImageDialog();
        let contentWizard = new ContentWizard();
        // 1. Open existing content and double click in html-area
    ...
    it.skip(`GIVEN existing htmlarea-content with an inserted image is opened WHEN rollback version with 'Custom Width' THEN image-range is getting visible again`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
        let insertImageDialog = new InsertImageDialog();
        let contentWizard = new ContentWizard();
