/**
 * Created on 27.04.2022 updated on 07.08.2026
 */
...
describe('filter.by.modifier.spec: tests for filter panel', function () {
    ...
    it(`Preconditions: new folder with 'English (en)' should be added`, async () => {
    ...
    it(`WHEN Filter Panel is opened THEN expected aggregation groups should be present`, async () => {
    ...
    it(`WHEN just created folder is filtered THEN The number of items in the 'Last Modified By' aggregation checkbox should be 1`, async () => {
        let filterPanel = new FilterPanel();
        ...
        await filterPanel.waitForCheckboxDisplayed(
            appConst.FILTER_PANEL_AGGREGATION_BLOCK.LAST_MODIFIED_BY,
    ...
    it(`WHEN just created folder is filtered THEN The number of items in the 'Owner' aggregation checkbox should be 1`, async () => {
        let filterPanel = new FilterPanel();
        // 1. Type the name of folder in Filter Input:
        await studioUtils.findAndSelectItem(FOLDER_NAME_1);
        // 2. Verify that "Me" checkbox is displayed in 'Last Modified By' block in Filter Panel
