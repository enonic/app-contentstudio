/**
 * Created on 24.05.2022  updated on 07.08.2026
 */
...
describe('filter.by.owner.spec: tests for filtering by', function () {
    ...
    it('GIVEN just created user is logged in WHEN wizard for new folder has been opened THEN expected compact collaboration-name should be displayed in the wizard toolbar', async () => {
    ...
    it("GIVEN just created user added a folder with En language WHEN wizard for new child folder has been opened THEN 'English' language should be present in the wizard by default", async () => {
        // 1. user is logged in:
        await studioUtils.navigateToContentStudioApp(USER.displayName, USER.password);
        ...
        assert.equal(
            ...
            'English language should be selected by default in wizard for new child content',
        );
        await studioUtils.doCloseAllWindowTabsAndNavigateToHome();
        await studioUtils.doLogout();
    });

    it('GIVEN SU is logged in WHEN Filter Panel has been opened THEN owner-selector should be present in aggregations view', async () => {
    ...
    it("WHEN existing user has been selected in 'Owner' selector in Filter Panel THEN only the content that were created by this user should be present in the grid", async () => {
        let filterPanel = new FilterPanel();
    ...
    it("WHEN existing user has been selected in 'Modified by' selector THEN only the content that were modified by this user should be present in the grid", async () => {
        let filterPanel = new FilterPanel();
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. SU is logged in:
        await studioUtils.navigateToContentStudioApp('su', 'password');
    ...
    it("GIVEN 'Last Modified by' selector is expanded WHEN checkbox for the user has been unchecked in List Options THEN 'Clear Filter' icon gets not visible", async () => {
        let filterPanel = new FilterPanel();
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. SU is logged in:
        await studioUtils.navigateToContentStudioApp('su', 'password');
    ...
    it("GIVEN an user has been checked in 'Last Modified by' selector WHEN 'Clear Filter' has been clicked THEN the user-checkbox gets unchecked in List Options", async () => {
        let filterPanel = new FilterPanel();
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. SU is logged in:
        await studioUtils.navigateToContentStudioApp('su', 'password');
