...
describe('contentItem.preview.toolbar.spec: tests for preview toolbar', function () {
    ...
    it(`WHEN existing folder has been selected THEN 'Version history' button should be displayed in the preview-toolbar`, async () => {
    ...
    it(`WHEN existing folder has been unselected THEN Preview toolbar gets not visible`, async () => {
    ...
    it(`GIVEN existing folder has been published WHEN 'Versions history' button has been clicked THEN 'Online' status should be displayed in the widget`, async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        let contentItemPreviewPanel = new ContentItemPreviewPanel();
        ...
        assert.equal(label, 'Published', 'Versions History Button should be with the default label');
    });

    it(`WHEN published folder has been modified THEN 'Modified' label should appear in Versions History Button`, async () => {
        let contentItemPreviewPanel = new ContentItemPreviewPanel();
        ...
        assert.equal(
            label,
            appConst.CONTENT_STATUS.MODIFIED,
            `'Modified' label should appear in Versions History Button`,
        );
    ...
    it.skip(`GIVEN modified folder is selected WHEN the folder has been published THEN 'Show changes' button gets hidden in the item preview toolbar`, async () => {
        let contentItemPreviewPanel = new ContentItemPreviewPanel();
        let contentBrowsePanel = new ContentBrowsePanel();
        let contentPublishDialog = new ContentPublishDialog();
