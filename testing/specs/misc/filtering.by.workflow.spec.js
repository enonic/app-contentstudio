...
describe('filtering.by.workflow.spec: tests for filter panel', function () {
    ...
    it(`WHEN existing 'work in progress' folder is filtered THEN The number of items in the grid and number in the 'work in progress' aggregation should be equal`, async () => {
        ...
        await filterPanel.waitForCheckboxDisplayed(
            appConst.FILTER_PANEL_AGGREGATION_BLOCK.WORKFLOW,
        ...
        await filterPanel.waitForCheckboxNotDisplayed(
            appConst.FILTER_PANEL_AGGREGATION_BLOCK.WORKFLOW,
            appConst.WORKFLOW_STATE.READY_FOR_PUBLISHING,
        );
        //5. Get the number in the 'Work in progress' checkbox:
        let numberInCheckbox = await filterPanel.getNumberOfItemsInAggregationView(
            'Workflow',
            appConst.WORKFLOW_STATE.WORK_IN_PROGRESS,
        );
    ...
    it(`GIVEN existing 'work in progress' folder is filtered WHEN the folder has been marked as ready THEN "Work in progress" checkbox gets not visible`, async () => {
        let filterPanel = new FilterPanel();
        let contentBrowsePanel = new ContentBrowsePanel();
        ...
        //await filterPanel.waitForCheckboxDisplayed(appConst.FILTER_PANEL_AGGREGATION_BLOCK.WORKFLOW, appConst.WORKFLOW_STATE.READY_FOR_PUBLISHING);
        ...
        //assert.equal(numberInCheckbox, items.length, "The number of items in the grid must be the same as the number in the label in Filter Panel");
    });
