...
describe('content.xdata.outbound.dependency.spec: checks outbound dependency for a content with x-data(image)', function () {
    ...
    //1. Open the wizard:
    await
    studioUtils.selectSiteAndOpenNewWizard(SITE.displayName, 'double1_1');
    await
    contentWizard.typeDisplayName(CONTENT_WITH_XDATA);
    //2. Enable the x-data and select an image::
    await
    contentWizard.clickOnXdataToggler();
    await
    xDataImageSelector.filterOptionsAndSelectImage(IMAGE_DISPLAY_NAME);
    await
    ...
    await
    xDataImageSelector.waitForImageSelected();
        });
    ...
    await
    studioUtils.selectContentAndOpenWizard(CONTENT_WITH_XDATA);
    await
    ...
    studioUtils.selectSiteAndOpenNewWizard(SITE.displayName, 'double1_1');
    await
    contentWizard.typeDisplayName(CONTENT_WITH_XDATA_2);
    // click on '+' and enable the x-data
    await
    contentWizard.clickOnXdataToggler();
    await
    studioUtils.saveAndCloseWizard();
    //2. Reopen the content:
    await
    studioUtils.selectContentAndOpenWizard(CONTENT_WITH_XDATA_2);
    //3. Verify that, x-data with image-selector should be enabled and image selector should be visible
    await
    xDataImageSelector.waitForImageOptionsFilterInputVisible();
        });
    ...
    await
    contentWizard.waitForSaveButtonVisible();
        });
