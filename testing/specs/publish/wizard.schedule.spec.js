...
describe('Wizard page - verify schedule form`', function () {
    ...
    await
    studioUtils.selectAndOpenContentInWizard(TEST_FOLDER.displayName);
    await
    contentWizard.openPublishMenuAndPublish();
    let result = await
    contentWizard.isWizardStepByTitlePresent(SCHEDULE_STEP_TITLE);
    ...
    let author = await
    contentWizard.getContentAuthor();
    ...
    await
    studioUtils.doUnPublishInWizard();
    //3. Verify the toolbar and status:
    let result = await
    contentWizard.waitForWizardStepByTitleNotVisible(SCHEDULE_STEP_TITLE);
    assert.isTrue(result, "Schedule menu item gets not visible in the step-navigator");

    result = await
    contentWizard.waitForScheduleFormNotVisible();
