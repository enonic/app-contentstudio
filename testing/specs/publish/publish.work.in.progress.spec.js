...
describe('publish.work.in.progress.spec - publishes work in progress content', function () {
    ...
    it("GIVEN 'Work in progress' folder is opened WHEN 'Publish...' menu item has been pressed AND 'Mark as ready' menu item clicked THEN the content gets 'Ready for publishing'", async () => {
    ...
    it(`GIVEN existing 'work in progress' site has been published in wizard WHEN Unpublish button has been pressed THEN 'Confirm Value' dialog with with '1' number is loaded`, async () => {
        let contentWizard = new ContentWizard();
        let contentPublishDialog = new ContentPublishDialog();
    ...
    it(`GIVEN existing unpublished site is selected WHEN 'Publish...' button has been pressed AND Include children has been clicked THEN the site should be published`, async () => {
        let contentPublishDialog = new ContentPublishDialog();
        ...
        assert.equal(
            actualMessage,
            appConst.NOTIFICATION_MESSAGES.TWO_ITEMS_PUBLISHED,
            "'2 items are published.' should appear",
        );
    });

    it("GIVEN existing published site is opened WHEN a text component has been inserted THEN site's state should be 'Modified' and 'Work in progress' icon should be displayed in the grid", async () => {
        ...
        assert.equal(
            ...
            "'Work in progress' should be displayed in browse panel",
        );
    });
