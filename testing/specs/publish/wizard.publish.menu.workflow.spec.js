...
describe('wizard.publish.menu.workflow.spec - publishes and unpublishes single folder in wizard', function () {
    ...
    it(`GIVEN name input is filled in WHEN display name input is empty THEN only 'Create Task' menu item should be enabled`, async () => {
    ...
    it(`GIVEN 'Marked as Ready' folder is opened WHEN 'Publish...' button has been pressed AND the folder has been published THEN 'UNPUBLISH' button gets visible in the toolbar`, async () => {
        let contentWizard = new ContentWizard();
    ...
    it(`GIVEN existing 'published' folder is opened WHEN publish menu has been expanded THEN 'Request Publishing...' menu item should be disabled AND 'Create Task...' is enabled`, async () => {
        let contentWizard = new ContentWizard();
    ...
    it(`GIVEN existing 'Modified' folder is opened WHEN publish menu has been expanded THEN 'Request Publishing' menu item should be enabled AND 'Create Issue' is enabled`, async () => {
        let contentWizard = new ContentWizard();
    ...
    it(`GIVEN existing 'modified' content is opened WHEN 'unpublish' button has been pressed AND it confirmed in the modal dialog THEN 'UNPUBLISHED' status should appear in the wizard`, async () => {
        let contentWizard = new ContentWizard();
        ...
        assert.ok(
            /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(result),
        ...
        //  Verify the bug  https://github.com/enonic/app-contentstudio/issues/11045
