...
describe('request.publish.dialog.validation.spec - opens request publish modal dialog and checks validation message', function () {
    ...
    await
    studioUtils.findAndSelectItem(appConst.TEST_FOLDER_NAME);
    //expand the Publish Menu and select 'Request Publishing...' menu item
    await
    contentBrowsePanel.openPublishMenuAndClickOnRequestPublish();
    //2. click on 'Include children items'
    await
    createRequestPublishDialog.clickOnIncludeChildItems(appConst.TEST_FOLDER_WITH_IMAGES);
    ...
    await
    createRequestPublishDialog.waitForInvalidIconNotDisplayed();
})
    ;
    ...
    //1. folder with children(all the children are valid) is selected:
    await
    studioUtils.findAndSelectItem(appConst.TEST_FOLDER_2_NAME);
    //expand the Publish Menu and select 'Request Publishing...' menu item
    await
    contentBrowsePanel.openPublishMenuAndClickOnRequestPublish();
    //2. click on 'Include children items':
    await
    createRequestPublishDialog.clickOnIncludeChildItems(appConst.TEST_FOLDER_2_DISPLAY_NAME);
    ...
    await
    createRequestPublishDialog.waitForInvalidIconDisplayed();
})
    ;
    ...
    await
    createRequestPublishDialog.pressEscKey();
    await
    await
    createRequestPublishDialog.waitForDialogClosed();
