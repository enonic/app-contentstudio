...
describe('browse.panel.sorticon.spec, do sort a folder and check a sort-icon in the grid`', function () {
    ...
    await
    contentBrowsePanel.clickOnSortButton();
    await
    sortContentDialog.clickOnMenuButton();
    //2. 'Manually sorted' menu item has been clicked:
    await
    sortContentDialog.selectSortMenuItem(appConstant.sortMenuItem.MANUALLY_SORTED);
    ...
    let sortingType = await
    contentBrowsePanel.getSortingIcon(appConstant.TEST_FOLDER_WITH_IMAGES);
    ...
    contentBrowsePanel.clickOnSortButton();
    await
    sortContentDialog.waitForDialogVisible();
    //2. Expand the menu:
    await
    sortContentDialog.clickOnMenuButton();
    //3. 'Published date' sorted menu item has been clicked
    await
    sortContentDialog.selectSortMenuItem(appConstant.sortMenuItem.PUBLISHED_DATE, 'ascending');
    await
    sortContentDialog.clickOnSaveButton();
    await
    contentBrowsePanel.pause(1000);
    //4. reopen sort dialog:
    await
    contentBrowsePanel.clickOnSortButton();
    let sortingType = await
    sortContentDialog.getSelectedOrder();
