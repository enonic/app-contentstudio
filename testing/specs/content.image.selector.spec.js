...
describe('content.image.selector: Image content specification', function () {
    ...
    await
    imageSelectorForm.clickOnDropdownHandle();
    //3. Verify expanded options:
    studioUtils.saveScreenshot("selector_flat_mode");
    let nameImages = await
    imageSelectorForm.getFlatModeOptionImageNames();
    ...
    //1. Open wizard with Image Selector:
    await
    studioUtils.selectSiteAndOpenNewWizard(SITE.displayName, appConstant.contentTypes.IMG_SELECTOR_1_1);
    //2. Type the folder-name:
    await
    imageSelectorForm.doFilterOptions(FOLDER_WITH_FILES);
    //3. Verify that expected options are present in the expanded list:
    let optionsName = await
    imageSelectorForm.getFlatModeOptionImageNames();
