...
describe('textline.content.config.spec:  verifies `max-length value config for textLine`', function () {
    ...
    //1. open new wizard:
    await
    studioUtils.selectSiteAndOpenNewWizard(SITE.displayName, appConstant.contentTypes.TEXTLINE_MAX_LENGTH);
    //2. Type the text(less than MAX LENGTH)
    await
    textLine.typeText('hello');
    await
    textLine.pause(1000);
    let result = await
    textLine.isValidationRecordingVisible();
    ...
    await
    textLine.typeText('123456789123');
    let result = await
    textLine.waitForValidationRecording();
