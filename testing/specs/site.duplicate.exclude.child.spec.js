...
describe('site.duplicate.exclude.child.spec:  select a site exclude child and duplicate the site', function () {
    ...
    await
    studioUtils.doAddFolder(CHILD_FOLDER);
    await
    studioUtils.typeNameInFilterPanel(CHILD_FOLDER.displayName);
    await
    contentBrowsePanel.waitForContentDisplayed(CHILD_FOLDER.displayName);
        });
    ...
    await
    contentDuplicateDialog.clickOnDuplicateButton();
    await
    contentDuplicateDialog.waitForDialogClosed();
    //3. Verify that the site has been copied with its children:
    await
    studioUtils.findAndSelectItem(SITE.displayName + "-copy");
    studioUtils.saveScreenshot("site_and_children_duplicated");
    await
    contentBrowsePanel.clickOnExpanderIcon(SITE.displayName + "-copy");
    await
    contentBrowsePanel.waitForContentDisplayed('_templates');
    await
    contentBrowsePanel.waitForContentDisplayed(CHILD_FOLDER.displayName);
        });
    ...
    await
    studioUtils.findAndSelectItem(SITE.displayName + "-copy-2");
    studioUtils.saveScreenshot("site_duplicated_no_child");
    let isDisplayed = await
    contentBrowsePanel.isExpanderIconPresent(SITE.displayName + "-copy-2");
