...
describe("wizard.toolbar.shortcut.spec Wizard toolbar - tests for shortcuts for button spec", function () {
    ...
    it(`GIVEN folder-wizard is opened WHEN 'Ctrl+s' has been pressed THEN folder should be saved`, async () => {
        ...
        await contentWizard.waitForExpectedNotificationMessage(
            appConst.itemSavedNotificationMessage(DISPLAY_NAME),
        );
    });
    ...
    it.skip(`GIVEN folder-wizard is opened WHEN 'Alt+w' have been pressed THEN wizard should be closed and grid is loaded`, async () => {
        let contentWizard = new ContentWizard();
    ...
    it(`GIVEN existing folder is opened AND the display name has been updated WHEN 'Ctrl+Enter' have been pressed THEN the content should be saved and the wizard closes`, async () => {
        let contentWizard = new ContentWizard();
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Open existing folder:
        await studioUtils.selectAndOpenContentInWizard(DISPLAY_NAME);
