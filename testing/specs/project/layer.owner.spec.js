...
describe('layer.owner.spec - ui-tests for user with layer-Owner role ', function () {
    ...
    it("GIVEN user with 'Owner'-layer role is logged in WHEN 'inherited' site has been selected THEN 'Mark as ready' button should appear in the browse toolbar", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Do log in with the user-owner and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
    ...
    it("GIVEN user with 'Owner'-layer role is logged in WHEN the user attempts to open existing site in draft THEN expected page should be loaded", async () => {
        // 1. Do Log in with the user:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
        //user's context should be loaded by default now!
        // 2. load existing site from the current layer:
        let url =
            'http://localhost:8080/admin/com.enonic.app.contentstudio/site/preview' +
            `/${LAYER_DISPLAY_NAME}/draft/${SITE_NAME}`;
    ...
    it('Precondition: Existing site has been marked as ready in the parent project', async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Do Log in with 'SU':
        await studioUtils.navigateToContentStudioApp();
    ...
    it("GIVEN 'inherited' site was marked as ready in the parent project THEN workflow in child layer should be 'Ready for publishing'", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Do log in with the user-owner and navigate to Content Browse Panel:
    ...
    it("GIVEN user with 'Owner'-layer role is logged in WHEN wizard page with htmlArea has been opened THEN 'Source' button should be displayed in the htmlArea toolbar", async () => {
        // 1. Do log in with the user-owner and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioCloseProjectSelectionDialog(USER.displayName, PASSWORD);
    ...
    it("GIVEN user with 'Owner' role do double click on the inherited site WHEN 'Localize' button has been clicked THEN PCV gets unlocked in the wizard step", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        ...
        assert.equal(
            ...
            'Expected notification message should be displayed',
        );
    ...
    it("GIVEN localized site has been opened WHEN 'Reset' button has been clicked THEN Preview button should enabled in the ItemPreview toolbar", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        let contentWizard = new ContentWizard();
        // 1. Do log in with the user-owner and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioCloseProjectSelectionDialog(USER.displayName, PASSWORD);
