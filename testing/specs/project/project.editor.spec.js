/**
 * Created on 15.06.2020.  updated on 10.08.2026
 */
...
describe("project.editor.spec - ui-tests for an user with 'Editor' role", function () {
    ...
    it('GIVEN new project wizard dialog is opened WHEN existing user has been added as Editor THEN expected user should be selected in Project Roles form', async () => {
    ...
    it('Precondition 2: new site should be created by the SU in the parent project', async () => {
    ...
    it('WHEN current user has access to only one project THEN ProjectViewer button should not be clickable', async () => {
    ...
    it('GIVEN user with Editor role is logged in WHEN existing project has been selected THEN New...,Edit, Delete buttons should be disabled', async () => {
    ...
    it('GIVEN user with Editor role is logged in WHEN New Content Dialog has been opened THEN only Folder and Shortcut types are allowed for Editor role', async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
    ...
    it("GIVEN user with Editor role is logged in WHEN new folder has been saved THEN 'Mark as Ready' should be as default action in Publish Menu", async () => {
    ...
    it("GIVEN user with 'Editor' role is logged in WHEN existing folder(work in progress) has been published THEN the folder gets Published", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Do log in with the user-editor and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
