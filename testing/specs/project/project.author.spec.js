...
describe('project.author.spec - ui-tests for user with Author role', function () {
    ...
    it("GIVEN new project wizard is opened WHEN existing user has been added as 'Author' THEN expected user should be selected in Project Roles form", async () => {
    ...
    it('Precondition 2: new site should be created in the just created project', async () => {
        // 1. Do Log in with 'SU':
        await studioUtils.navigateToContentStudioApp();
    ...
    it("GIVEN user with 'Author' role is logged in WHEN the user attempts to open existing site in draft THEN expected page should be loaded", async () => {
        ...
        let url =
            ...
            `/${PROJECT_DISPLAY_NAME}/draft/${SITE_NAME}`;
    ...
    it("GIVEN user with 'Author' role is logged in WHEN existing project has been selected THEN New...,Edit, Delete buttons should be disabled", async () => {
    ...
    it("GIVEN user with 'Author' role is logged in WHEN new folder has been saved THEN 'Mark as Ready' should be as default action in Publish Menu", async () => {
        let contentWizard = new ContentWizard();
    ...
    it("GIVEN user with 'Author' role is logged in WHEN existing folder has been marked as ready THEN Publish menu item should be disabled for an user with Author role", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Do log in with the user-author and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
    ...
    it("GIVEN user with 'Author' role is logged in WHEN existing folder has been selected and Publish Request has been created THEN 'Publish Now' button should be disabled on the last stage", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
