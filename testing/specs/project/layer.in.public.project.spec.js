/**
 * Created on 23.07.2020.  updated on 06.08.2026
 */
...
describe('layer.in.public.project.spec - ui-tests for layer in existing project', function () {
    ...
    it(`Preconditions: new project(with Norsk (no) language) and 'Private' access mode should be added`, async () => {
        ...
        await projectUtils.saveTestProject({
            ...
            accessMode: appConst.PROJECT_ACCESS_MODE.PUBLIC,
        });
    });

    it("GIVEN Buttons: 'Copy language from parent' has been clicked and 'Save' pressed WHEN layer's context has been switched THEN expected language should be displayed in the project context", async () => {
        ...
        let editProjectAndLanguageStep = new EditProjectAndLanguageStep();
        await editProjectAndLanguageStep.waitForLoaded();
        // 3. Click on 'Copy language from parent' button:
        await editProjectAndLanguageStep.clickOnCopyFromParentButton(PROJECT_DISPLAY_NAME);
        // 4. Save the layer:
        await editProjectAndLanguageStep.clickOnNextButton();
    ...
    it("GIVEN existing layer is opened WHEN the language has been updated THEN expected language should be displayed in the layer's context", async () => {
        let settingsBrowsePanel = new SettingsBrowsePanel();
        // 1. Open the layer:
        await settingsBrowsePanel.clickOnRowByDisplayName(LAYER_DISPLAY_NAME);
        await settingsBrowsePanel.clickOnEditButton();
        ...
        await editProjectAndLanguageStep.clickOnRemoveSelectedLanguageIcon();
        // 3. insert the text in Options Filter Input then click on filtered option then click on OK
        await editProjectAndLanguageStep.selectLanguage(appConst.LANGUAGES.EN);
        await editProjectAndLanguageStep.clickOnNextButton();
        let editProjectNameStep = new EditProjectNameStep();
        await editProjectNameStep.waitForLoaded();
        await editProjectNameStep.clickOnNextButton();
    ...
    it('WHEN existing parent project is selected THEN Delete button should be disabled', async () => {
        let settingsBrowsePanel = new SettingsBrowsePanel();
    ...
    it("WHEN existing parent project and its child project are selected THEN 'Delete' button should be disabled in the browse-toolbar", async () => {
        let settingsBrowsePanel = new SettingsBrowsePanel();
    ...
    it("WHEN project and its layer are selected THEN 'Delete' menu item should be disabled in context-menu", async () => {
        let settingsBrowsePanel = new SettingsBrowsePanel();
        // 1.parent project and its child project are selected:
        await settingsBrowsePanel.clickOnCheckboxAndSelectRowByName(PROJECT_DISPLAY_NAME);
        await settingsBrowsePanel.clickOnCheckboxAndSelectRowByName(LAYER_DISPLAY_NAME);
        // 2. Verify that 'Delete' button is disabled in browse-panel:
        await settingsBrowsePanel.rightClickOnProjectItemByDisplayName(PROJECT_DISPLAY_NAME);
    ...
    it('GIVEN existing layer has been deleted WHEN content mode has been switched THEN Default context should be loaded', async () => {
        let settingsBrowsePanel = new SettingsBrowsePanel();
        let contentBrowsePanel = await studioUtils.switchToContentMode();
        ...
        assert.equal(
            actualContextName1,
            LAYER_DISPLAY_NAME + ' (en)',
            "layer's context should be loaded before the deleting of the layer",
        );
        assert.equal(
            actualContextName2,
            PROJECT_DISPLAY_NAME + ' (no)',
            'Parent context should be loaded after deleting its layer',
        );
    });

    it('Post conditions: the project should be deleted', async () => {
