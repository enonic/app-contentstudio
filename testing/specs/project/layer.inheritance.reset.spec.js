/**
 * Created on 16.11.2020. updated on 06.08.2026
 */
...
describe('layer.inheritance.reset.spec - tests for Reset button in wizard toolbar', function () {
    ...
    it(`Precondition 1 - parent project with private access mode should be created`, async () => {
    ...
    it('Precondition 2: new site should be created in existing parent project', async () => {
    ...
    it('Precondition 3: new layer(English (en)) should be created in the existing project', async () => {
        await studioUtils.openSettingsPanel();
    ...
    it("GIVEN site is selected AND 'Edit' button has been pressed WHEN 'Localize' button has been pressed THEN 'Reset' button should appear in the wizard toolbar", async () => {
        // 1. Select the layer's context:
        let contentWizard = new ContentWizard();
        await studioUtils.openProjectSelectionDialogAndSelectContext(LAYER_DISPLAY_NAME);
        // 2. Click on 'Edit' button then save the changes in the opened site:
    ...
    it("GIVEN 'Reset' button has been pressed AND Confirmation dialog is opened WHEN 'No' button has been clicked THEN the site remains localized", async () => {
        let contentWizard = new ContentWizard();
    ...
    it("GIVEN 'Reset' button has been pressed AND Confirmation dialog is opened WHEN 'Yes' button has been clicked THEN the site should be reverted to the inherited state", async () => {
        let contentWizard = new ContentWizard();
    ...
    it("GIVEN not localized site has been marked as ready WHEN 'Reset' button has been clicked THEN the site's workflow state should be reverted to Work in progress", async () => {
        let contentWizard = new ContentWizard();
        ...
        assert.equal(
            actualStatus,
            appConst.ICON_WORKFLOW_STATE.IN_PROGRESS,
    ...
    it("GIVEN controller has been selected in the inherited site WHEN 'Reset' button has been pressed THEN the site should be reverted to the inherited state", async () => {
        let contentWizard = new ContentWizard();
        // 1. layer's context should be loaded automatically.
        ...
        // TODO bug https://github.com/enonic/app-contentstudio/issues/11231
        // Preview panel is not refreshed after resetting localized content
