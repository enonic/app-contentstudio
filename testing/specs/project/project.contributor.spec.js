/**
 * Created on 09.06.2020.  updated on 11.08.2026
 */
...
describe('project.contributor.spec - ui-tests for user with Contributor role', function () {
    ...
    it("Precondition 2: 'Work in Progress' and Ready folders should be created in the just created project", async () => {
    ...
    it("GIVEN user with 'Contributor' role is logged in WHEN existing site(controller is not selected) is opened THEN Page Controller should be disabled", async () => {
        ...
        await contentWizardPanel.waitForContentFormLocked();
    });

    it('GIVEN contributor user is logged in WHEN existing project has been selected THEN New...,Edit, Delete buttons should be disabled', async () => {
    ...
    // TODO bug  https://github.com/enonic/app-contentstudio/issues/11259
    it.skip("GIVEN user -'Contributor' is logged in WHEN existing folder has been selected  AND versions panel opened THEN 'Revert' button should be disabled", async () => {
    ...
    // https://github.com/enonic/app-contentstudio/issues/1934
    // // TODO bug  https://github.com/enonic/app-contentstudio/issues/11259
    it.skip("GIVEN user -'Contributor' is logged in WHEN existing folder has been opened AND versions panel opened THEN 'Revert' button should be disabled", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
    ...
    it("GIVEN user with 'Contributor' role is logged in WHEN existing folder(Ready to publish) has been selected THEN 'Publish' menu item should be disabled for users with 'Contributor' role", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Do log in with the user-contributor and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
    ...
    it("GIVEN user with 'Contributor' role is logged in WHEN existing folder(Work in Progress) has been selected THEN 'Publish', 'Request Publishing' menu items should be disabled for users with 'Contributor' role", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Do log in with the user-contributor and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
        // 2. Select existing folder(ready to publish):
        ...
        assert.ok(
            menuItems.includes(appConst.PUBLISH_MENU.PUBLISH) === false,
            'Publish menu item should not be present',
        );
    });

    it("GIVEN user with 'Contributor' role is logged in WHEN double click on an existing folder THEN the folder should be opened in the new browser tab AND all inputs should be disabled", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        let contentWizardPanel = new ContentWizardPanel();
        // 1. Do log in with the user-contributor and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
    ...
    it("GIVEN user with 'Contributor' role is logged in WHEN double click on an existing folder THEN 'Create Issue' default action should be present in publish-menu", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        let contentWizardPanel = new ContentWizardPanel();
        // 1. Do log in with the user-contributor and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
        ...
        // 3. Verify that Request publishing is default action in publish menu:
        await contentWizardPanel.waitForRequestPublishingButtonDisplayed();
    });

    it.skip("GIVEN user with 'Contributor' role is logged in WHEN existing folder(Ready to publish) has been selected and Publish Request has been created THEN 'Publish Now' button should be disabled on the last stage", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
    ...
    it("GIVEN user with 'Contributor' role is logged in WHEN 2 folders have been selected THEN 'Duplicate' button should be disabled", async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        // 1. Do log in with the user-contributor and navigate to Content Browse Panel:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
        // 2. Select 2 folders:
