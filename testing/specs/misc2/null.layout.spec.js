...
describe("null.layout.spec - test for layout-controller that returns null ", function () {
    ...
    it(`WHEN null layout has been inserted THEN 'layout is empty' text should be displayed in LiveEdit`, async () => {
    ...
    it(`GIVEN a part with text input in its config has been saved as fragment WHEN text has been typed THEN Save button gets enabled`, async () => {
        let contentWizard = new ContentWizard();
        let pageComponentView = new PageComponentView();
