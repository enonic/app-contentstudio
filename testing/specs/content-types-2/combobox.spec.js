...
describe('combobox.content.spec: tests for comboBox content', function () {
    ...
    it(`GIVEN wizard for new not required ComboBox 0:0 is opened WHEN name input have been filled THEN the content gets valid`,
        async () => {
            ...
            await comboBoxForm.typeInFilterAndClickOnOption("option A");
            ...
            assert.isFalse(isNotValid, 'the content should be valid, because combobox input is not required');
        });
    ...
    it("GIVEN existing content with a selected option(ComboBox 1:1)is opened WHEN the option has been removed THEN the content gets not valid",
        async () => {
            let comboBoxForm = new ComboBoxForm();
            let contentWizard = new ContentWizard();
            //1. open new wizard and fill in the name input:
            ...
            let isNotValid = await contentWizard.isContentInvalid();
            assert.isTrue(isNotValid, 'the content should be not valid, because combobox input is required');
    ...
    it("GIVEN wizard for new ComboBox 2:4 is opened WHEN 2 options have been selected THEN the content gets valid",
        async () => {
            ...
            await comboBoxForm.typeInFilterAndClickOnOption("option A");
            await comboBoxForm.typeInFilterAndClickOnOption("option B");
    ...
    it("GIVEN existing ready for publishing content with 2 selected option(ComboBox 2:4)is opened WHEN one option has been removed THEN the content gets not valid",
        async () => {
            let comboBoxForm = new ComboBoxForm();
            let contentWizard = new ContentWizard();
            //1. open new wizard and fill in the name input:
            ...
            assert.isTrue(result.includes('option A'), "Expected option should be selected");
            assert.isTrue(result.includes('option B'), "Expected option should be selected");
