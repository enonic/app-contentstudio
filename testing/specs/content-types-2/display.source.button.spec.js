...
describe("display.source.button.spec - tests for user with Content Manager Expert role and  Display Source button", function () {
    ...
    it(`Precondition 1: new user with 'Content Manager Expert' and Author roles should be created`, async () => {
        ...
        let roles = [
            appConst.SYSTEM_ROLES.ADMIN_CONSOLE,
            appConst.SYSTEM_ROLES.CM_APP_EXPERT,
            "Default - Author",
        ];
        USER = builder.buildUser(
            userName,
            appConst.PASSWORD.MEDIUM,
            builder.generateEmail(userName),
            roles,
        );
    ...
    it("Precondition 2: new site should be created by SU", async () => {
        ...
        let editPermissionsChooseApplyChangesStep =
            new EditPermissionsChooseApplyChangesStep();
        ...
        SITE = contentBuilder.buildSite(siteName, null, [
        ...
        await editPermissionsGeneralStep.filterAndSelectPrincipal(
            USER.displayName,
        );
        await editPermissionsGeneralStep.showAceMenuAndSelectItem(
            USER.displayName,
            appConst.permissions.FULL_ACCESS,
        );
        ...
        let number =
            await editPermissionsSummaryStep.getNumberFromApplyChangesButton();
        ...
        assert.equal(
            number,
            1,
            "Expected number of changes should be displayed in the button: 1",
        );
    });

    it("GIVEN user with roles 'Author ' and 'Content Manager Expert' is logged in WHEN content with html area has been opened THEN 'Source' button should be displayed in the htmlArea toolbar", async () => {
    ...
    it("GIVEN user with roles 'Author' and 'Content Manager Expert' is signing in WHEN existing site has been opened THEN 'Edit configurator' and 'remove application' icons should be hidden", async () => {
