...
describe(`article.allow.child.cfg.spec: tests for 'allow-child-content-type' in content type schema`, function () {
    ...
    it(`GIVEN article with 'A non-existing content type is allowed' in cfg is selected WHEN 'New content' modal dialog has been opened THEN 'Upload' button should not be displayed`, async () => {
        let contentWizard = new ContentWizard();
        let contentBrowsePanel = new ContentBrowsePanel();
        let newContentDialog = new NewContentDialog();
    ...
    it(`GIVEN article with 'All media types are allowed via wild card' in cfg is selected WHEN 'New content' modal dialog has been opened THEN 'Upload' button should be displayed`, async () => {
        let contentWizard = new ContentWizard();
        let contentBrowsePanel = new ContentBrowsePanel();
        let newContentDialog = new NewContentDialog();
        ...
        await studioUtils.selectSiteAndOpenNewWizard(
            IMPORTED_SITE_NAME,
            appConst.contentTypes.ARTICLE_ALLOW_MEDIA_WILD_CARD,
        );
        // 2. Fill in the name input, save and close the wizard:
