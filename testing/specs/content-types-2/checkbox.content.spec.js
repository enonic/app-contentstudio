...
describe("checkbox.content.spec: tests for content with checkbox", function () {
    ...
    it("GIVEN wizard for new 'checkbox(0:1)' content is opened WHEN the checkbox has been clicked THEN should be checked after clicking on it", async () => {
        ...
        await studioUtils.selectSiteAndOpenNewWizard(
            IMPORTED_SITE_NAME,
            appConst.contentTypes.CHECKBOX_0_1,
        );
        ...
        assert.ok(
            isSelected,
            "Checkbox should be checked after clicking on it",
        );
    ...
    it("WHEN existing 'checkbox' content has been reopened THEN the checkbox should be selected", async () => {
        let checkBoxForm = new CheckBoxForm();
    ...
    it("GIVEN wizard for content with required checkbox is opened WHEN name input has been filled in AND Save button pressed THEN validation recording gets visible", async () => {
        let checkBoxForm = new CheckBoxForm();
        let contentWizard = new ContentWizard();
        // 1. open new wizard and fill in the name input:
        await studioUtils.selectSiteAndOpenNewWizard(
            IMPORTED_SITE_NAME,
            appConst.contentTypes.CHECKBOX_1_1,
        );
        ...
        assert.equal(
            recording,
            appConst.THIS_FIELD_IS_REQUIRED,
