...
describe('htmlarea.cke.toolbar.spec: tests for toolbar in html-area(CKE editor)', function () {
    ...
    const PASTE_MODE_FORMATTED = 'Paste mode: formatted text';
    const PASTE_MODE_PLAIN = 'Paste mode: plain text';

    it(`GIVEN 'insert special characters' dialog is opened WHEN 'Soft hyphen' special character has been clicked THEN '&shy' should be present in the htmlArea`, async () => {
    ...
    it(`GIVEN wizard for 'htmlArea' is opened WHEN 'Find and replace' button has been clicked THEN search form should be loaded`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
        // 1.  new wizard for htmlArea content is opened:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
    ...
    it(`GIVEN 'htmlArea' content is opened WHEN 'insert image' icon has been clicked THEN 'Insert Image Dialog' should appear`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
    ...
    it(`GIVEN 'htmlArea' content is opened WHEN 'insert anchor' icon has been clicked THEN 'Insert Anchor Dialog' should appear`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
    ...
    it(`GIVEN 'insert macro' dialog is opened WHEN 'Cancel' button has been pressed THEN 'Insert Macro Dialog' should be closed`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
    ...
    it(`WHEN 'htmlArea' content is opened THEN Increase Indent, Bulleted List, Align Right,Table buttons should be present on the toolbar`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
    ...
    it(`GIVEN wizard for 'htmlArea' is opened AND 'Insert Link' button has been pressed WHEN 'url-link' has been inserted, THEN correct data should be in CKE`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
    ...
    it("GIVEN wizard for 'htmlArea' is opened WHEN 'format-dropdown' handle has been clicked THEN expected options should appear", async () => {
        let htmlAreaForm = new HtmlAreaForm();
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
        await htmlAreaForm.pause(1000);
    ...
    it(`GIVEN wizard for 'htmlArea' is opened and text is entered WHEN 'Heading 1' option has been selected THEN expected text should be in area`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
        // 1. Open new wizard:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
        await htmlAreaForm.pause(1000);
    ...
    it(`GIVEN wizard for 'htmlArea 0:1' is opened WHEN 'Table' menu item has been clicked THEN drop down menu with table should appear`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
    ...
    // Verifies: Paste mode button in the HTML Area toolbar throws a TypeError
    it(`GIVEN 'htmlArea' content is opened WHEN 'Paste Mode' icon has been clicked THEN the tooltip should switch to 'plain text' AND no errors should be logged in the browser console`, async () => {
        let htmlAreaForm = new HtmlAreaForm();
        // 1. new wizard for htmlArea content is opened:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
        await htmlAreaForm.pause(1000);
        await htmlAreaForm.showToolbar();
        // 2. Verify the initial state of the button:
        let tooltip = await htmlAreaForm.getPasteModeButtonTooltip();
        assert.equal(tooltip, PASTE_MODE_FORMATTED, "'formatted text' should be the initial paste mode");
        // 3. Drain the console log collected while the wizard was loading:
        await studioUtils.getBrowser().getLogs('browser');
        // 4. 'Paste mode' button has been clicked:
        await htmlAreaForm.showToolbarAndClickOnPasteModeButton();
        await studioUtils.saveScreenshot('cke_paste_mode_plain');
        // 5. Verify that the paste mode is switched to plain text:
        tooltip = await htmlAreaForm.getPasteModeButtonTooltip();
        assert.equal(tooltip, PASTE_MODE_PLAIN, "'plain text' paste mode should be switched on");
        // 6. Verify that the plugin does not throw an error:
        let logs = await studioUtils.getBrowser().getLogs('browser');
        let pluginErrors = logs.filter((log) => log.level === 'SEVERE' && log.message.includes('pasteModeSwitcher'));
        assert.equal(pluginErrors.length, 0, 'Paste mode button should not throw: ' + JSON.stringify(pluginErrors));
        // 7. Click on the button again:
        await htmlAreaForm.showToolbarAndClickOnPasteModeButton();
        // 8. Verify that the paste mode is switched back to formatted text:
        tooltip = await htmlAreaForm.getPasteModeButtonTooltip();
        assert.equal(tooltip, PASTE_MODE_FORMATTED, "'formatted text' paste mode should be switched on");
    });
