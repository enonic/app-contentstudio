/**
 * Created on 24.01.2019.  updated on 13.08.2026
 */
...
describe("optionset.confirmation.spec: checks for 'confirmation' dialog when deleting an existing or new item-set", function () {
    ...
    it(`GIVEN 'option 1' is selected in the single selector WHEN help text icon has been clicked THEN expected help text get visible`, async () => {
    ...
    it(`WHEN 'Option 1'  has been clicked THEN 'Option 1' should be selected`, async () => {
        let singleSelectionOptionSet = new SingleSelectionOptionSet('my selector');
        // 1. Open wizard for new option set:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.OPTION_SET_HELP_TEXT);
    ...
    it(`GIVEN wizard for new content with Option Set is opened WHEN name input has been filled AND Save button pressed THEN validation recording should appear`, async () => {
        ...
        assert.ok(
            result,
            "The Content should be invalid, because the required option is not selected in 'single selection'",
        );
        ...
        assert.equal(
            recording,
            appConst.VALIDATION_MESSAGE.SINGLE_SELECTION_OPTION_SET,
            'Expected message gets visible',
        );
    });

    it(`GIVEN existing content with Option Set is opened AND text has been typed in the option with htmlArea WHEN the option has been unchecked AND confirmed THEN Save button should gets disabled again`, async () => {
        let contentWizard = new ContentWizard();
        ...
        /// Verify the bug  https://github.com/enonic/app-contentstudio/issues/10858
        await multiSelectionOptionSet.clickOnOption('Option 3');
        // 5. Verify that Notification dialog is loaded:
        let messages = await multiSelectionOptionSet.waitForNotificationMessages();
        let expectedMsg = 'The fields inside unselected option will be cleared on save!';
        assert.ok(messages.includes(expectedMsg), 'Expected message should be displayed');
    ...
    it(`GIVEN existing invalid content with Option Set is opened WHEN an option in required selection has been selected AND Save button pressed THEN the content gets valid`, async () => {
        let contentWizard = new ContentWizard();
        let singleSelectionOptionSet = new SingleSelectionOptionSet();
        // 1. Open the new wizard:
        await studioUtils.selectAndOpenContentInWizard(CONTENT_NAME_1);
        ...
        assert.ok(
            result === false,
            "The Content should be valid, because an option is selected in the required 'single selection'",
        );
        await contentWizard.waitAndClickOnSave();
    ...
    it(`GIVEN existing valid content with Option Set is opened WHEN the selected checkbox in multi selection has been unselected THEN the content gets invalid`, async () => {
        let contentWizard = new ContentWizard();
        let multiSelectionOptionSet = new MultiSelectionOptionSet();
        // 1. Open the new wizard:
        await studioUtils.selectAndOpenContentInWizard(CONTENT_NAME_1);
    ...
    it(`GIVEN option set with dirty fields WHEN 'Reset' menu item has been clicked THEN 'Notification Dialog' should be loaded`, async () => {
        let singleSelectionOptionSet = new SingleSelectionOptionSet();
        // 1. Open the new wizard:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.OPTION_SET);
        ...
        await studioUtils.saveScreenshot('item_set_notification');
        let expectedMsg = 'The fields inside unselected option will be cleared on save!';
        assert.ok(messages.includes(expectedMsg), 'Expected message should be displayed');
        // 7. Verify that 'Save' button gets disabled again
        await contentWizard.waitForSaveButtonEnabled();
    ...
    it(`GIVEN wizard for new 'option set' is opened  AND 'Add My Item-set' has been clicked WHEN text typed in the second item-set AND 'remove' item-set button has been pressed THEN 'Confirmation Dialog' should appear`, async () => {
        let singleSelectionOptionSet = new SingleSelectionOptionSet();
    ...
    it(`GIVEN wizard for new 'option set' is opened  AND 'Add My Item-set' has been clicked WHEN text typed in the second item-set AND 'remove' item-set button has been pressed THEN 'Confirmation Dialog' should not appear`, async () => {
        let singleSelectionOptionSet = new SingleSelectionOptionSet();
        let confirmationMask = new ConfirmationMask();
        // 1. Open the new wizard:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.OPTION_SET);
        ...
        assert.ok(
            result === false,
            'Confirmation mask Dialog should not be loaded, because new item-set has no dirty fields',
        );
    });

    it(`GIVEN Confirmation mask Dialog is opened WHEN 'Esc' key has been pressed THEN 'Confirmation mask Dialog' closes`, async () => {
        let singleSelectionOptionSet = new SingleSelectionOptionSet();
        let confirmationMask = new ConfirmationMask();
        // 1. Open the new wizard:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.OPTION_SET);
    ...
    it(`GIVEN wizard for new 'option set' is opened  AND 'Single Selection' form has been clicked WHEN all required inputs have been filled THEN red icon gets not visible(content is valid)`, async () => {
        let singleSelectionOptionSet = new SingleSelectionOptionSet();
        let contentWizard = new ContentWizard();
        // 1. Open new wizard:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.OPTION_SET);
    ...
    it(`GIVEN wizard for new 'option set' is opened WHEN name has been typed AND Save button pressed THEN Saved button should appear`, async () => {
        let contentWizard = new ContentWizard();
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.OPTION_SET);
        let displayName = contentBuilder.generateRandomName('optionset');
    ...
    it("GIVEN wizard for new 'article' is opened WHEN article's title and display name are filled in THEN the content should be invalid", async () => {
        let contentWizard = new ContentWizard();
    ...
    // Verifies: Incorrect behaviour of validation when two required text inputs/ text area/ text line are present in the wizard #2616
    // https://github.com/enonic/app-contentstudio/issues/2616
    it("GIVEN wizard for new 'article' is opened WHEN article's title and display name are filled in AND Save button has been pressed THEN the content should be invalid", async () => {
        let contentWizard = new ContentWizard();
        let articleForm = new ArticleForm();
        // 1. Open new wizard for article-content:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.ARTICLE);
    ...
    // Verifies: Incorrect behaviour of validation when two required text inputs/ text area/ text line are present in the wizard #2616
    // https://github.com/enonic/app-contentstudio/issues/2616
    it("GIVEN wizard for new 'article' is opened WHEN both required inputs are filled in AND 'Save' button has been pressed THEN the content should be valid", async () => {
        let contentWizard = new ContentWizard();
        let articleForm = new ArticleForm();
        // 1. Open new wizard for article-content:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.ARTICLE);
