...
describe('occurrences.double.spec: tests for content with Double inputs', function () {
    ...
    it(`GIVEN wizard for content with a not required 'double' is opened WHEN max safe value has been typed THEN validation message should not be displayed`,
        async () => {
            ...
            assert.isFalse(isInvalid, "Double content should be valid");
        });
    ...
    it(`GIVEN wizard for 'double 2:4' content is opened WHEN values in both inputs have been typed and one is invalid THEN red border should be present in the second double input`,
        async () => {
            let doubleForm = new DoubleForm();
            let contentWizard = new ContentWizard();
            ...
            //4. Verify thar red border appears in the second double input:
