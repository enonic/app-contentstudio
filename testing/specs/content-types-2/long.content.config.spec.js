...
describe('long.content.config.spec:  verifies `Min/max value config for Long`', function () {
    ...
    it(`GIVEN wizard for required 'Long(min 1,max 10)' is opened WHEN number in the allowed range has been typed THEN validation message should not be present`, async () => {
    ...
    it(`GIVEN wizard for required 'Long(min 1,max 10)' is opened WHEN value less than min has been typed THEN validation record should be visible`, async () => {
        let longForm = new LongForm();
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.LONG_MIN_MAX);
    ...
    it(`GIVEN wizard for required 'Long(min 1,max 10)' is opened WHEN value more than max has been typed THEN validation record should appear`, async () => {
        let longForm = new LongForm();
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.LONG_MIN_MAX);
    ...
    it(`GIVEN wizard for required 'Long(min 1,max 10)' is opened WHEN max value has been typed THEN input validation record should not be visible`, async () => {
        let longForm = new LongForm();
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.LONG_MIN_MAX);
    ...
    it(`GIVEN wizard for required 'Long(min 1,max 10)' is opened WHEN min value has been typed THEN input validation recording should not be visible`, async () => {
        let longForm = new LongForm();
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.LONG_MIN_MAX);
    ...
    it(`GIVEN invalid value is typed AND validation recording is displayed WHEN valid value has been entered THEN the validation recording gets hidden`, async () => {
        let longForm = new LongForm();
    ...
    it(`GIVEN wizard for 'Long(min 1,max 10,required)' is opened WHEN invalid value has been typed THEN 'Save' button should be disabled`, async () => {
        let longForm = new LongForm();
        let contentWizardPanel = new ContentWizardPanel();
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.LONG_MIN_MAX);
