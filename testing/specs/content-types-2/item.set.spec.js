...
describe("item.set.spec: tests for content with Item Set", function () {
    ...
    it("GIVEN ItemSet form has been added in the wizard WHEN 'add above' menu item has been clicked THEN 'Expand all' button gets visible", async () => {
        ...
        await studioUtils.selectSiteAndOpenNewWizard(
            IMPORTED_SITE_NAME,
            appConst.contentTypes.ITEM_SET_0_0,
        );
    ...
    it("GIVEN ItemSet form has been added in the wizard WHEN the required inputs has been filled in THEN the content gets valid and red border is not displayed", async () => {
        let itemSetForm = new ItemSetForm();
        let contentWizard = new ContentWizard();
        await studioUtils.selectSiteAndOpenNewWizard(
            IMPORTED_SITE_NAME,
        ...
        assert.ok(
            isRedBorderDisplayed === false,
            "red border should not be displayed",
        );
        ...
        assert.equal(
            ...
            "Validation recording should be displayed",
        );
        await contentWizard.waitAndClickOnSave();
        await contentWizard.waitForNotificationMessage();
    ...
    it("GIVEN a text has been inserted in the ItemSet form WHEN 'Delete' item-set menu item has been clicked THEN 'Delete ItemSet' button should appears", async () => {
        let itemSetForm = new ItemSetForm();
        let contentWizard = new ContentWizard();
    ...
    it("WHEN wizard for new content with ItemSet(0:0) is opened THEN 'Add' button should be displayed", async () => {
        let itemSetForm = new ItemSetForm();
        let contentWizard = new ContentWizard();
        await studioUtils.selectSiteAndOpenNewWizard(
            IMPORTED_SITE_NAME,
        ...
        assert.ok(
            isInvalid === false,
            "the content with Item Set should be valid",
        );
    ...
    it("GIVEN a new ItemSet(0:0) with only the name field filled in and saved, WHEN the 'Add' button is pressed, THEN the content becomes invalid.", async () => {
        let itemSetForm = new ItemSetForm();
        let contentWizard = new ContentWizard();
        await studioUtils.selectSiteAndOpenNewWizard(
            IMPORTED_SITE_NAME,
        ...
        assert.ok(
            isInvalid,
            "Item Set content should be invalid, required inputs are present in the set",
        );
        await contentWizard.waitAndClickOnSave();
        await contentWizard.waitForNotificationMessage();
    ...
    it("GIVEN existing invalid ItemSet(0:0) content is opened WHEN required inputs have been filled in the form THEN the content gets valid", async () => {
        let itemSetForm = new ItemSetForm();
        let contentWizard = new ContentWizard();
        ...
        assert.ok(
            isInvalid === false,
            "the content with Item Set should be valid now",
        );
    });

    it("WHEN existing invalid content with empty ItemSet is opened THEN 'Expand/Collapse' button are not displayed visible", async () => {
        let itemSetForm = new ItemSetForm();
        let contentWizard = new ContentWizard();
        ...
        assert.ok(
            isInvalid,
            "the content remains invalid after clicking on Collapse button",
        );
    ...
    it("GIVEN the second form has been added WHEN required inputs have been filled in the second form THEN the content remains invalid", async () => {
        let itemSetForm = new ItemSetForm();
        let contentWizard = new ContentWizard();
    ...
    it("GIVEN existing content with single empty form is opened AND the second form has been added WHEN both forms have been filled THEN the content gets valid now", async () => {
        let itemSetForm = new ItemSetForm();
        let contentWizard = new ContentWizard();
        ...
        assert.ok(
            isInvalid === false,
            "the content with Item Set becomes valid.",
        );
