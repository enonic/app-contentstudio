...
describe("remove_app.in.site.with.descriptor.spec: replace an application and check the selected controller", function () {
    ...
    it(`WHEN the selected application has been replaced with another application THEN controller from the first application remains visible in PCV AND we can reset the controller`, async () => {
    ...
    it(`GIVEN app selector has been expanded WHEN checkbox for the selected app has been unchecked THEN the selected option should not be displayed in the form`, async () => {
        let siteFormPanel = new SiteFormPanel();
        // 1. Existing site is opened:
        await studioUtils.selectAndOpenContentInWizard(SITE.displayName);
    ...
    it(`GIVEN app has been removed in the site the site is reopened WHEN 'Page widget' has been opened THEN Invalid Controller should be displayed in Inspect tab`, async () => {
        let contentWizard = new ContentWizard();
    ...
    it(`GIVEN site with a selected controller is opened AND a page component is selected in PCV WHEN expand the menu for the 'main region' controller and click on 'Reset' menu item THEN 'No controller is selected' message gets visible `, async () => {
        let contentWizard = new ContentWizard();
        let pageInspectionPanel = new PageInspectionPanel();
        let pageComponentsWizardStepForm = new PageComponentsWizardStepForm();
        // 1. Existing site is opened:
        ...
        assert.equal(
            ...
            `${NO_SELECTED_CONTROLLER_MSG} message should be displayed`,
        );
    });
