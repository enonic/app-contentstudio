/**
 * Created on 13.07.2022  updated on 01.07.2026
 */
...
describe('insert.content.link.anchor.spec: insert content link into htmlArea', function () {
    ...
    it("GIVEN Content tab is active AND 'Add anchor' button has been clicked and text inserted WHEN 'Insert' button has been pressed THEN expected text should be present in htmlArea", async () => {
        ...
        await insertLinkDialogContentPanel.selectTargetInContentSelector(IMPORTED_SITE_NAME);
    ...
    it("GIVEN 'Add parameters' button has been clicked then parameter form is filled WHEN Insert button has been pressed THEN expected text should be present in htmlArea", async () => {
        let htmlAreaForm = new HtmlAreaForm();
        let insertLinkDialogContentPanel = new InsertLinkDialogContentPanel();
        // 1. Open new wizard for htmlArea content:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
    ...
    it('WHEN Anchor with a parameter has been inserted THEN expected text should be present in htmlArea', async () => {
        let htmlAreaForm = new HtmlAreaForm();
        let insertLinkDialogContentPanel = new InsertLinkDialogContentPanel();
        // 1. Open new wizard for htmlArea content:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
    ...
    it("WHEN 'Insert Content link' dialog is reopened THEN whitespaces should be trimmed in anchor and parameter inputs", async () => {
        let htmlAreaForm = new HtmlAreaForm();
        let insertLinkDialogContentPanel = new InsertLinkDialogContentPanel();
        // 1. Open new wizard for htmlArea content:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
    ...
    // Verify  https://github.com/enonic/app-contentstudio/issues/10981
    it("GIVEN 'Insert Content link' dialog is reopened WHEN 'Parameter name' input has been cleared THEN 'All parameters must have a name' - message should be displayed in the modal dialog", async () => {
        let htmlAreaForm = new HtmlAreaForm();
        let insertLinkDialogContentPanel = new InsertLinkDialogContentPanel();
        // 1. Open new wizard for htmlArea content:
        await studioUtils.selectSiteAndOpenNewWizard(IMPORTED_SITE_NAME, appConst.contentTypes.HTML_AREA_0_1);
        ...
        await insertLinkDialogContentPanel.clearParameterNameInput(0);
        ...
        assert.equal(
            message,
            appConst.VALIDATION_MESSAGE.LINK_PARAMETERS,
