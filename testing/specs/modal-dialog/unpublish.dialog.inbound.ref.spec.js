...
describe('unpublish.dialog.inbound.ref.spec - Tests for inbound references in Unpublish modal dialog', function () {
    ...
    it(`Preconditions: published folder and  shortcut and one not published shortcut should be created`, async () => {
    ...
    it(`GIVEN Unpublish content dialog is opened WHEN 'Ignore inbound references' link has been clicked THEN 'Unpublish' button gets enabled in the dialog`, async () => {
    ...
    it(`GIVEN Unpublish content dialog is opened WHEN 'Show references' link has been clicked THEN 'Not published' referenced content should not be displayed in the filtered grid`, async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        let contentUnpublishDialog = new ContentUnpublishDialog();
        ...
        // Verifies the bug https://github.com/enonic/app-contentstudio/issues/10862
        // BUG - "Show references" in the Unpublish dialog shows offline dependencies
        assert.ok(refItems.length === 1, `'Not published' referenced content should not be displayed`);
        assert.equal(
            refItems[0],
            SHORTCUT_NAME,
            'Only published ref-content should be displayed in the filtered grid ',
        );
        let status = await contentBrowsePanel.getContentStatus(SHORTCUT_NAME);
        assert.equal(
            status,
            appConst.CONTENT_STATUS.ONLINE,
            `'Online' status should be displayed for the filtered content`,
        );
    });

    it(`GIVEN 'Ignore inbound references' is displayed in Unpublish content dialog WHEN 'Show references' link has been clicked AND referenced content has been unpublished THEN 'Unpublish' button gets enabled`, async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        let contentUnpublishDialog = new ContentUnpublishDialog();
        // 1.Click on the image, that was selected in the shortcut
        await studioUtils.findAndSelectItem(FOLDER_NAME);
