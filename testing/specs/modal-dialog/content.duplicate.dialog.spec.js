/**
 * Created on 29.05.2018.  update on 30.06.2026
 */
...
describe('content.duplicate.dialog.spec: Content Duplicate Dialog specification', function () {
    ...
    it(`GIVEN 2 folders with child items are selected WHEN 'Duplicate...' button has been clicked THEN expected display names should be present in the dialog`,
        async () => {
            let contentBrowsePanel = new ContentBrowsePanel();
            let contentDuplicateDialog = new ContentDuplicateDialog();
    ...
    it(`GIVEN 'Content Duplicate' dialog is opened WHEN 'Include child' has been unchecked THEN 'Dependant items' gets hidden`,
        async () => {
            let contentBrowsePanel = new ContentBrowsePanel();
            let contentDuplicateDialog = new ContentDuplicateDialog();
            // 1. Select the folder(12 child items) :
            await studioUtils.findAndSelectItem(appConst.TEST_FOLDER_NAME);
            await contentBrowsePanel.clickOnDuplicateButtonAndWait();
            let isIncludeCheckboxSelected = await contentDuplicateDialog.isIncludeCheckboxSelected(
                appConst.TEST_DATA.TEST_FOLDER_IMAGES_1_DISPLAY_NAME);
            assert.ok(isIncludeCheckboxSelected, "'Include child items' checkbox should be selected bu default");
            // 2. Click on the checkbox and exclude child items
            await contentDuplicateDialog.clickOnIncludeChildCheckbox(appConst.TEST_DATA.TEST_FOLDER_IMAGES_1_DISPLAY_NAME);
            await contentDuplicateDialog.waitForIncludeChildCheckboxDisplayed(appConst.TEST_DATA.TEST_FOLDER_IMAGES_1_DISPLAY_NAME);
            ...
            isIncludeCheckboxSelected =
                await contentDuplicateDialog.isIncludeCheckboxSelected(appConst.TEST_DATA.TEST_FOLDER_IMAGES_1_DISPLAY_NAME);
            assert.ok(!isIncludeCheckboxSelected, "'Include child items' checkbox should be unchecked");
        });
