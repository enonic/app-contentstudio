...
describe('wizard.compare.versions.dialog - open the dialog and verify elements', function () {
    ...
    await
    settingsStepForm.filterOptionsAndSelectLanguage('English (en)');
    await
    ...
    await
    wizardVersionsWidget.clickOnCompareWithCurrentVersionButton(1);
    //4. Verify that modal dialog is loaded:
    await
    compareContentVersionsDialog.waitForDialogOpened();
    //5. Right 'Revert' menu-button should be disabled:
    await
    compareContentVersionsDialog.waitForRightRevertMenuButtonDisabled();
    //6. Left 'Revert' menu-button should be enabled:
    await
    compareContentVersionsDialog.waitForLeftRevertMenuButtonEnabled();
})
    ;
    ...
    await
    compareContentVersionsDialog.clickOnLeftRevertMenuButton();
    //5. 'Revert' menu-item should be present in the expanded menu:
    await
    compareContentVersionsDialog.waitForLeftRevertMenuItemDisplayed();
})
    ;
    ...
    await
    contentWizard.pressEscKey();
    //3. Verify that the modal dialog is closed:
    await
    compareContentVersionsDialog.waitForDialogClosed();
})
    ;
    ...
    await
    compareContentVersionsDialog.clickOnCancelButtonTop();
    //3. Verify that the modal dialog is closed:
    await
    compareContentVersionsDialog.waitForDialogClosed();
})
