...
describe("edit.permissions.accessselector.spec:  Select 'Custom...' permissions and add 'Create' operation", function () {
    ...
    await
    studioUtils.doAddFolder(FOLDER);
})
    ;
    ...
    await
    studioUtils.findAndSelectItem(FOLDER.displayName);
    await
    studioUtils.openBrowseDetailsPanel();
    //2. Open Edit Permissions dialog and uncheck the 'Inherit' checkbox:
    await
    userAccessWidget.clickOnEditPermissionsLinkAndWaitForDialog();
    await
    editPermissionsDialog.clickOnInheritPermissionsCheckBox();
    //3. Go to 'Content Manager App' entry and open ACE-menu then select 'Custom...' menu item:
    await
    editPermissionsDialog.showAceMenuAndSelectItem(appConstant.roleName.CONTENT_MANAGER_APP, appConstant.permissions.CUSTOM);
    await
    editPermissionsDialog.pause(500);
    //4. Click on 'Create' permission-toggle and allow the operation:
    studioUtils.saveScreenshot("edit_perm_dlg_custom_permissions");
    await
    editPermissionsDialog.clickOnPermissionToggle(appConstant.roleName.CONTENT_MANAGER_APP,
    ...
    await
    studioUtils.selectAndOpenContentInWizard(FOLDER.displayName);
    await
    contentWizardPanel.clickOnAccessTabBarItem();
    //2. Click on 'Content Manager App' entry- operations get visible:
    await
    accessStepForm.clickOnEntryRow(appConstant.roleDisplayName.CONTENT_MANAGER_APP);
    studioUtils.saveScreenshot("wizard_permissions_operations");
    //3. Get operations in 'Content Manager App' entry:
    let actualOperations = await
    accessStepForm.getPermissionOperations(appConstant.roleDisplayName.CONTENT_MANAGER_APP);
    ...
    let isAllowed = await
    accessStepForm.isOperationAllowed(appConstant.roleDisplayName.CONTENT_MANAGER_APP, 'Read');
    assert.isTrue(isAllowed, '`Read` operation should be allowed(green)');
    isAllowed = await
    accessStepForm.isOperationAllowed(appConstant.roleDisplayName.CONTENT_MANAGER_APP, 'Create');
    ...
    await
    editPermissionsDialog.isOperationDenied(appConstant.roleName.CONTENT_MANAGER_APP, 'Create');
    await
    editPermissionsDialog.isOperationAllowed(appConstant.roleName.CONTENT_MANAGER_APP, 'Read');
        });
