...
describe('close.issue.no.items.spec: create an issue than does not have items, close the issue and reopen it again', function () {
    ...
    it(`WHEN new issue without items has been created THEN 'Back To Issues Button' should be displayed in the Items-tab`, async () => {
    ...
    it(`GIVEN existing 'open' issue is clicked and Issue Details dialog is opened WHEN 'Close Issue' button has been pressed THEN the issue gets 'Closed'`, async () => {
        ...
        await issueDetailsDialog.waitForExpectedNotificationMessage(
            appConst.NOTIFICATION_MESSAGES.ISSUE_CLOSED_MESSAGE,
        );
    });

    it(`GIVEN 'closed' issue is clicked and Issue Details dialog is opened WHEN 'Reopen Issue' button has been pressed THEN the tsk gets 'Open'`, async () => {
        let issueListDialog = new IssueListDialog();
        let issueDetailsDialog = new IssueDetailsDialog();
        await studioUtils.openIssuesListDialog();
        ...
        await issueDetailsDialog.waitForExpectedNotificationMessage(
            appConst.NOTIFICATION_MESSAGES.ISSUE_OPENED_MESSAGE,
        );
    ...
    it(`GIVEN Issue Details dialog is opened WHEN 'Esc' key has been pressed THEN modal dialog should be closed`, async () => {
        let issueListDialog = new IssueListDialog();
        let issueDetailsDialog = new IssueDetailsDialog();
    ...
    it(`GIVEN Issue Details dialog is opened WHEN 'back-button' key has been pressed THEN Issues list dialog gets visible`, async () => {
        let issueListDialog = new IssueListDialog();
        let issueDetailsDialog = new IssueDetailsDialog();
        // 1. Open Issues List
        await studioUtils.openIssuesListDialog();
