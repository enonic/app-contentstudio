...
describe('task.details.dialog.spec: add a comment and check CommentsTabItem', function () {
    ...
    it(`GIVEN Task Details dialog is opened WHEN comment has been typed in the area THEN Comment button gets enabled`,
        async() = > {
            ...
            let commentsTab = new IssueDetailsDialogCommentsTab();
    await
    studioUtils.openIssuesListDialog();
    await
    issueListDialog.clickOnIssue(TASK_TITLE);
    await
    taskDetailsDialog.waitForDialogOpened();
    await
    commentsTab.typeComment(MY_COMMENT);
    studioUtils.saveScreenshot("issue_comment_typed");
    //'Comment' button gets enabled
    await
    commentsTab.waitForCommentButtonEnabled();
        });
    ...
    commentsTab.typeComment(MY_COMMENT);
    await
    commentsTab.clickOnCommentButton();
    let message = await
    taskDetailsDialog.waitForNotificationMessage();
    ...
    await
    commentsTab.waitForCommentButtonDisabled();
        });
    ...
    taskDetailsDialog.waitForDialogOpened();
    await
    commentsTab.clickOnEditCommentMenuItem(MY_COMMENT);
    await
    commentsTab.updateComment(MY_COMMENT, newText);
    await
    commentsTab.clickOnSaveCommentButton(MY_COMMENT);
    let result = await
    commentsTab.isCommentPresent(newText);
