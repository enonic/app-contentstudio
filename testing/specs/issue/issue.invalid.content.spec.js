...
describe('issue.invalid.content.spec: create a issue with invalid content', function () {
    ...
    it("GIVEN Create Issue dialog is opened with 2 parent folders WHEN 'Include children' icons have been clicked AND 'Create Issue' button has been pressed THEN all child items should be included in the 'Items' tab", async () => {
        ...
        // 7. Verify that 'All' checkbox is displayed in the dialog:
        await issueDetailsDialogItemsTab.waitForAllDependantsCheckboxDisplayed();
        // 8. Verify that expected number of items is displayed in the Items tab-link:
    ...
    it(`GIVEN existing folder with one invalid child item is selected WHEN 'Create Issue' menu item has been selected and issue created THEN '10' number should be in 'Items' on IssueDetailsDialog`, async () => {
        let issueDetailsDialog = new IssueDetailsDialog();
        let createIssueDialog = new CreateIssueDialog();
        let contentBrowsePanel = new ContentBrowsePanel();
        ...
        await issueDetailsDialog.waitForDialogLoaded();
        await issueDetailsDialog.pause(1000);
        // 5. 12 items should be in the issue-details dialog:
        let result = await issueDetailsDialog.getNumberInItemsTab();
