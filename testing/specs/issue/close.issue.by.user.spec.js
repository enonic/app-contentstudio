...
describe('close.issue.by.user.spec: create a issue for user and close it', function () {
    ...
    it(`GIVEN SU selects a folder and opens 'Create Issue...' dialog WHEN new issue has been assigned to the just created user THEN new issue should be loaded in IssueDetailsDialog`, async () => {
    ...
    it("WHEN the user is logged in THEN 'Assigned to Me' button should be displayed in issueListDialog", async () => {
        // 1. user is logged in:
        ...
        // 3. Verify the selected option in the selector: Assigned to Me
        let result = await issueListDialog.getSelectedOptionInTypeFilter();
        assert.ok(result.includes('Assigned to Me'));
        await studioUtils.doCloseAllWindowTabs();
        await studioUtils.navigateToHomePage();
    ...
    it("GIVEN the user is logged in WHEN the user closed his issue THEN the issue should be 'Closed' in the issue details dialog", async () => {
        // 1. user is logged in:
        await studioUtils.navigateToContentStudioApp(USER.displayName, PASSWORD);
        let issueListDialog = new IssueListDialog();
        let issueDetailsDialog = new IssueDetailsDialog();
    ...
    it(`GIVEN SU is logged in WHEN the issue, just closed by the user has been loaded in Issue Details dialog THEN 'closed' info should be displayed`, async () => {
        // 1. SU is logged in:
        await studioUtils.navigateToContentStudioApp();
        let issueDetailsDialog = new IssueDetailsDialog();
        let issueListDialog = new IssueListDialog();
    ...
    it.skip('GIVEN the user has selected the folder WHEN the folder has been duplicated THEN expected owner should be displayed for this folder in Properties Widget', async () => {
        let contentBrowsePanel = new ContentBrowsePanel();
        ...
        assert.ok(
            actualUser.includes(USER.displayName),
            'Expected user name should be displayed in the version list item',
        );
