/**
 * Created on 6.04.2018.  updated on 03.07.2026
 */
...
describe('page.template.controller: select a controller in a template-wizard', function () {
    ...
    it(`GIVEN _templates folder is selected WHEN New button has been pressed THEN Media tab item should be disabled in the modal dialog`, async () => {
    ...
    it(`GIVEN template wizard is opened WHEN controller has been selected THEN Live Context Window should be loaded automatically`, async () => {
    ...
    //WidgetTemplateSection is not updated after creating new template #11018
    // https://github.com/enonic/app-contentstudio/issues/11018
    it(`GIVEN site and its page-template are opened WHEN the 'site' has been selected in supports (in template) THEN template should be applied in the site-wizard`, async () => {
        let contentWizard = new ContentWizard();
        ...
        assert.equal(
            ...
            'Expected template name should be displayed in the template widget',
        );
    ...
    it(`GIVEN an image is inserted in the page-template WHEN the site has been opened THEN image that was inserted in the template is displayed in the site as well`, async () => {
        let contentWizard = new ContentWizard();
    ...
    it(`GIVEN site is opened AND its page-template is opened WHEN 'support' option has been removed (in template) THEN controller-selector must appear in the site-wizard`, async () => {
        let pageTemplateForm = new PageTemplateForm();
        let contentWizard = new ContentWizard();
        let pageInspectionTab = new PageInspectionPanel();
        //1. Open the site:
    ...
    it(`Precondition - select 'site' option in Support selector`, async () => {
        let contentWizard = new ContentWizard();
        let pageTemplateForm = new PageTemplateForm();
        // 1. Open the template:
        await studioUtils.selectContentAndOpenWizard(TEMPLATE.displayName);
    ...
    it(`GIVEN site is opened and 'Customise page' button has been clicked WHEN new text component with a text has been inserted THEN the expected text should be displayed in the Live Edit`, async () => {
        let contentWizard = new ContentWizard();
        let pageInspectionPanel = new PageInspectionPanel();
        let liveFormPanel = new LiveFormPanel();
        let textComponentInspectionPanel = new TextComponentInspectionPanel();
        ...
        // 1. Open the site:
        await studioUtils.selectContentAndOpenWizard(SITE.displayName);
        // 2. Switch to 'live-edit' iframe and click on Page Settings
        await contentWizard.unlockSiteWithTemplate();
    ...
    //Template dropdown options not refreshed after deleting/creating template in Browse Panel #11020
    //https://github.com/enonic/app-contentstudio/issues/11020
    it(`GIVEN site is opened WHEN page-template has been deleted THEN site-wizard should be reset and  'No preview available' - should be displayed in  Wizard PreviewLabel`, async () => {
        let contentWizard = new ContentWizard();
        // 1. Open the site:
        await studioUtils.selectContentAndOpenWizard(SITE.displayName);
        await studioUtils.doSwitchToContentBrowsePanel();
        ...
        // 4. No preview available - should be displayed in  Wizard PreviewLabel
        ...
        let result = await pageInspectionPanel.getSelectedPageController();
        assert.equal(
            result,
            'Automatic',
            'Automatic option should be selected in the Page Controller selector after the template has been deleted',
        );
    });
    ...
    function selectTemplatesFolderAndClickOnNew() {
        ...
        let contentBrowsePanel = new ContentBrowsePanel();
        return studioUtils
            .findAndSelectItem(SITE.displayName)
            .then(() => {
