...
describe('Generate name for fragments  specification', function () {
    ...
    await
    pageComponentView.selectMenuItemAndCloseDialog(["Insert", "Text"]);
    await
    textComponentCke.switchToLiveEditFrame();
    //3. Open 'Insert Image' dialog and insert an image in htmlArea:
    await
    textComponentCke.clickOnInsertImageButton();
    await
    insertImageDialog.waitForDialogVisible();
    await
    insertImageDialog.filterAndSelectImage(TEST_IMAGE_NAME);
    await
    ...
    contentWizard.clickOnShowComponentViewToggler();
    await
    pageComponentView.openMenu("Text");
    await
    ...
    let fragmentContent = await
    contentWizard.getDisplayName();
    ...
    await
    imageInspectPanel.typeNameAndSelectImage(TEST_IMAGE_NAME);
    //4. Save the image-component as fragment:
    await
    contentWizard.clickOnShowComponentViewToggler();
    await
    pageComponentView.openMenu(TEST_IMAGE_NAME);
    await
    pageComponentView.clickOnMenuItem(appConstant.MENU_ITEMS.SAVE_AS_FRAGMENT);
    await
    contentWizard.pause(700);
    ...
    await
    pageComponentView.openMenu("Layout");
    await
    pageComponentView.clickOnMenuItem(appConstant.MENU_ITEMS.SAVE_AS_FRAGMENT);
    await
    contentWizard.pause(700);
