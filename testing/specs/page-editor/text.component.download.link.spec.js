...
describe('Text Component with CKE - insert download-link  specification', function () {
    ...
    await
    studioUtils.findAndSelectItem("server.sh");
    await
    contentBrowsePanel.clickOnMoveButton();
    await
    moveContentDialog.waitForOpened();
    await
    moveContentDialog.typeTextAndClickOnOption(SITE.displayName);
    await
    ...
    await
    studioUtils.insertDownloadLinkInCke("test", TEST_CONTENT_DISPLAY_NAME);
    await
    textComponentCke.switchToLiveEditFrame();
