...
describe('Text Component with CKE - insert link and table  specification', function () {
    ...
    await
    contentWizard.clickOnHideComponentViewToggler();
    await
    textComponentCke.switchToLiveEditFrame();
    //2. Click on 'Insert Table' menu-button:
    await
    textComponentCke.clickOnInsertTableButton();
    // menu item for inserting of Html-table gets visible:
    await
    textComponentCke.waitForTableDisplayedInCke();
        });
    ...
    await
    insertLinkDialog.typeText("url_link");
    await
    insertLinkDialog.typeUrl(NOT_VALID_URL);
    //2. Click on 'Insert" in the modal dialog:
    await
    insertLinkDialog.clickOnInsertButton();
    //Validation message gets visible:
    let result = await
    insertLinkDialog.waitForValidationMessage();
    ...
    textComponentCke.clickOnInsertLinkButton();
    await
    studioUtils.insertUrlLinkInCke("test", 'https://enonic.com');
    await
    textComponentCke.switchToLiveEditFrame();
    ...
    await
    studioUtils.clickOnElement('a=test');
    await
    contentBrowsePanel.pause(2000);
    let title = await
    studioUtils.getTitle();
    ...
    await
    contentItemPreviewPanel.waitForElementDisplayedInFrame('a=test');
})
    ;
    ...
    studioUtils.findAndSelectItem(SITE.displayName);
    await
    contentItemPreviewPanel.clickOnElementInFrame("a=test");
    studioUtils.saveScreenshot('enonic_not_loaded_in_preview_panel');
    //The Link gets not visible:
    let result = await
    contentItemPreviewPanel.waitForElementNotDisplayedInFrame("a=test");
    assert.isTrue(result, "The link should not be visible");
    await
    contentItemPreviewPanel.pause(2000);
    ...
    result = await
    contentItemPreviewPanel.waitForElementNotDisplayedInFrame("//h1[text()='Better customer journeys']");
