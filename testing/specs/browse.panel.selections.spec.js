...
describe('Browse panel selections spec`', function () {
    ...
    await
    contentBrowsePanel.clickOnRowByDisplayName(appConstant.TEST_FOLDER_WITH_IMAGES);
    let result = await
    contentBrowsePanel.getNameOfSelectedRow();
    ...
    let number = await
    contentBrowsePanel.getNumberOfSelectedRows();
    ...
    let number2 = await
    contentBrowsePanel.getNumberOfCheckedRows();
    assert.equal(number2, 0, "the number of checked rows is 0");
    let isVisible = await
    contentBrowsePanel.waitForSelectionTogglerVisible();
    ...
    await
    contentBrowsePanel.clickCheckboxAndSelectRowByDisplayName(appConstant.TEST_FOLDER_WITH_IMAGES);
    let number1 = await
    contentBrowsePanel.getNumberOfSelectedRows();
    ...
    contentBrowsePanel.clickCheckboxAndSelectRowByDisplayName(appConstant.TEST_FOLDER_WITH_IMAGES);
    await
    contentBrowsePanel.clickCheckboxAndSelectRowByDisplayName(appConstant.TEST_FOLDER_2_DISPLAY_NAME);
    let number = await
    contentBrowsePanel.getNumberOfSelectedRows();
