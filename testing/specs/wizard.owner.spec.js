...
describe('wizard.owner.spec - update and revert owner in wizard`', function () {
    ...
    await
    studioUtils.doAddFolder(TEST_FOLDER);
})
    ;
    ...
    await
    settingsForm.clickOnRemoveOwner("Super User");
    await
    settingsForm.filterOptionsAndSelectOwner('Anonymous User');
    await
    contentWizard.waitAndClickOnSave();
    //3. Open versions widget and revert the previous version:
    await
    contentWizard.openVersionsHistoryPanel();
    await
    ...
    let actualOwner = await
    settingsForm.getSelectedOwner();
