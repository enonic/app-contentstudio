...
export class CompareContentVersionsDialog
    ...
    private handleVersionChanged(isLeft?: boolean) {
        this.leftVersion = this.leftDropdown.getSelectedOption().getDisplayValue();

        this.rightVersion = this.rightDropdown.getSelectedOption().getDisplayValue();
    ...
    createVersionRevertButton(dropdown: Dropdown<ContentVersion>): Button {
        ...
        const revertAction = new Action(i18n('field.version.revert'))
            .onExecuted(() => {
                this.revertVersionCallback(this.contentId, dropdown.getSelectedOption().getDisplayValue());
            });
        ...
        return new GetContentVersionsRequest(this.contentId).setSize(-1).sendAndParse()
            .then((contentVersions: ContentVersions) => {
                ...
                if (!this.leftVersion || !this.rightVersion) {
                    ...
                    if (!this.leftVersion) {
                        this.leftVersion = latestOption.getDisplayValue();
                    }
                    if (!this.rightVersion) {
                        this.rightVersion = latestOption.getDisplayValue();
                    }
                }
                ...
                if (latestAlias.getDisplayValue().getId() !== this.rightVersion.getId()) {
                    this.rightVersion = latestAlias.getDisplayValue();
                }
    ...
    private updateAliases(isLeft: boolean): boolean {
        ...
        this.createAliases(dropdown.getOptions(), isLeft).forEach(alias => {
            ...
            if (alias.getDisplayValue().getAliasType() === selectedAliasType) {
                ...
                if (isLeft) {
                    this.leftVersion = alias.getDisplayValue();
                } else {
                    this.rightVersion = alias.getDisplayValue();
                }
            }
        });
    ...
    private disableLeftVersions() {
        ...
        this.leftDropdown.getOptions().slice().reverse().forEach((option: Option<ContentVersion>) => {
            ...
            if (option.getValue() === this.rightVersion.getId()) {
